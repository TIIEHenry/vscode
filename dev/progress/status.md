...
phase: M7
updated: 2026-09-08
summary: "FileMutationJoin / createScoped / Review 委托已修。D16 / D31 F4 仍开。官方三域 glob 已绿。D44–D90 / D92–D102 / D105 已闭。peek-before-advance、closeNonRootTabs notice 与 sessions MRU opener 回滚已挂。A2 仍 blocked。"
---
...
61. **D D100 / B D101 / A D102**（未关 D16）：peek-before-advance；closeEditors notice + finally；sessions MRU opener reject 回滚 cursor。
62. **D 槽 `reveal-item-acquire-throw-notice`（未关 D16）**：`conversation.revealItem` 的 `acquireSessionView` + reveal 包 try/catch → error notice；`finally` 仅 dispose 已拿到的 lease；void 调用点未改。[D105](deferred-gaps.md) **已闭**。
子 agent 发现的既有代码问题：
| ID | 来源 | 问题 |
|:---|:-----|:-----|
...
| [D92](deferred-gaps.md)–[D102](deferred-gaps.md) | B roster+close / D nav / A beside+mru | **closed** throw 回滚/notice；peek-before-advance；closeEditors notice；sessions opener 回滚 cursor；未关 D16 |
| [D105](deferred-gaps.md) | D 槽 `reveal-item-acquire-throw-notice` | **closed** `conversation.revealItem` acquire throw 已 notice；未关 D16；未改 roster throw 合同 / navigator void |
