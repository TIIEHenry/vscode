...
phase: M7
updated: 2026-09-08
summary: "D76 已闭：switchModel 空 resolvedModelId 回滚+gate。D16 仍开。D44–D73 已闭。A2 仍 blocked。"
---
...
44. **B 槽 `composer-switch-model-empty-rollback`（未关 D16 / 未碰 F4·A2·D26 / proto / `dev/loop`）**：`applySessionModelIndex` await 后 `!result.resolvedModelId.trim()` 走 restore + `showGateNotice`（`conversationLensDockModelFailed`）；未发明 `UniverseAgentSwitchModelResult.ok`。throw 测保留；补 empty resolve 回滚+gate。[D76](deferred-gaps.md) **已闭**。
