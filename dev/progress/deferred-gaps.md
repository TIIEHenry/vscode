...
created: 2026-08-30
updated: 2026-09-04
summary: "延期缺口 SSOT；D16 identity 已绿、Lens 叶宽+共享 harness 已补、断言仍红；进口界+deleteTurn 已隔离；D19/D21 已闭；D22 F3；D15 欠 W1"
---
...
| D16 | P2 | **conversation 单测基线红**：Lens 夹具已补（sessionBar 实测宽 + `lens.layout` 叶宽 + ResizeObserver）。过时断言已改：删非末会话计数 −1；300px 验 `is-narrow` 不是像素；permission seat aria 走 `Permission` 合同。**identity 已绿**（10/10）。**其余 Lens 夹具套已隔离**（`conversationLensLayoutHarness`：ResizeObserver loop 拦截 + layout flush，接到 reveal / trajectory / trajectoryUi；不改 `conversationLens.test.ts`）。stub `deleteTurn` 已改走 `createSession()`。**进口界空扫假绿已收**（四份测改扫 `src/` + `>=80`）。`conversationLens.test.ts` 仍可能有 DOM/codicon 断言债 | 启动级 / identity / stub / 进口界 / Lens 叶宽夹具 + 其余夹具 + 上述过时断言已修；本行剩 Lens 断言债 | `conversationLens.test.ts` 逐条分类；S2「Lens 全绿」以本行闭合为前提，或改为「不新增失败」 | M6 / conversation | open |
