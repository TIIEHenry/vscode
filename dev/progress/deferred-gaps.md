...
created: 2026-08-30
updated: 2026-09-06
summary: "延期缺口 SSOT；D16 Lens 断言债；D22 F3；D15 欠 W1；D23 confirmPairing；D24 其余 JSON RPC；D25 roster Create 仍不传本地 session id"
---
...
| D25 | P2 | **roster Create 仍不传本地 `clientSessionId`**：`conversationEngineRosterService.createEngineSessionRemote` 只发 `{ title: "New session" }`；连打依赖 encode 缺省 UUID 区分，不是稳定本地 session id。`recoverEngineSessionAfterCreateConflict` 把 List 失败 `catch` 成 `undefined`（与空列表同形）。引擎 store `Query does not return results` 真空未修 | 本轮禁止改 conversation / 引擎仓 | roster Create 带稳定本地 id；List 失败与空列表在 roster 侧分开；引擎 List 真空闭合 | conversation / engine | open |
