...
53. **B 槽 `hub-client-hydrate-catch`（未关 D16）**：Hub Channel Client `hydrate` try/catch；IPC reject 保留预 hydrate 默认（signedOut / idle / []）、不 fire 半应用 cache。未包 `setActiveHubBaseUrl`。未重开 D81。[D84](deferred-gaps.md) **已闭**。
子 agent 发现的既有代码问题：
...
| [D84](deferred-gaps.md) | B 槽 `hub-client-hydrate-catch` | **closed** Hub hydrate IPC reject 已吞；保留预 hydrate 默认；不 fire 半应用 cache；未关 D16 |
