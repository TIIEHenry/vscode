...
phase: M7
updated: 2026-09-12
summary: "关仓 MERGE_SHA 3c07195f6bf 已 push origin/agent-ide；compile-client 0；D405 手测仍开；D406/D407 已闭；D408 本槽 A 已写调度+测仍 open 至合入；勿占 D409；未升 PRD-008"
---
...
### 已合入（`MERGE_SHA`=`3c07195f6bf`；已 push `origin/agent-ide`；compile-client 0）
| 切片 | 提交 / 落点 |
|:-----|:------------|
...
| 槽 | 切片 | 状态 |
|:---|:-----|:---------|
| **A** | D408 reconnect-backoff | 本槽已写调度+测；D408 仍 open 至合入；未 commit |
...
| [D405](deferred-gaps.md)–[D408](deferred-gaps.md) | A/B/D 本波 | D405 **仍开**（§5 手测未跑；合入后 compile-client 类型已修为 0）；D406 **closed** KEEP leftover；D407 **closed** probeEngine；D408 本槽 A 已写调度+测，**仍 open 至合入**；勿占 D409 |
## 工位表（P0 盘点 · 2026-09-12 · 与 `git worktree list` 对照）
| 槽 | 路径 | 分支 | tip | 脏 | stash | 关仓状态 |
|----|------|------|-----|:--|:------|:---------|
| merge | `vscode-WorkTrees/merge` | `loop/merge` | `3c07195f6bf` | 干净（本提交前） | 0 | parked；已 push `origin/agent-ide` |
| A | `vscode-WorkTrees/A` | `loop/A` | `082c43506fa` | D408 调度+测未提交 | 0 | `connection-reconnect-backoff` 已写；未 commit |
| B | `vscode-WorkTrees/B` | `loop/B` | `3c07195f6bf` | 干净 | 0 | `idle`；已 cascade |
| C | `vscode-WorkTrees/C` | `loop/C` | `74f36dbacdf` | 未提交 `dev/loop` | 0 | `idle`；跳过 ff-only；勿 add |
| D | `vscode-WorkTrees/D` | `loop/D` | `3c07195f6bf` | 干净 | 0 | `idle`；已 cascade |
...
| **loop 切片** | **MERGE_SHA `3c07195f6bf`** 已 push。**D405 仍开**（§5 手测）。**D406/D407** 已闭。**D408** 本槽 A 已写调度+测，仍 open 至合入。不关 D10/D16/D18/D20/D12。勿占 **D409**。未升 PRD-008。 |
