...
phase: M7
updated: 2026-09-08
summary: "D16 / D31 F4 仍开。官方三域 glob 已绿。D44–D90 / D92–D118 已闭。通知 acquire throw 已吞。A2 仍 blocked。"
---
...
61. **D100–D117**（未关 D16）：peek / close / opener / stale / save·delete·cancel·Stop·Goal·Enqueue false notice / reveal / open-beside / primary bootstrap notice；Goal 断连+history 可点；permission/question roster false；SessionBar rename/delete 与 Sessions 侧栏 delete false notice。
62. **A 槽 `notifications-acquire-throw-catch`（未关 D16）**：`readPendingAttentionIds` 包 acquire try/catch；throw → [] + error notice + dispose 已拿 lease。测锁构造 / turn-settle 不崩。[D118](deferred-gaps.md) **已闭**。
子 agent 发现的既有代码问题：
| ID | 来源 | 问题 |
|:---|:-----|:-----|
...
| [D92](deferred-gaps.md)–[D118](deferred-gaps.md) | B roster+inbox+sessions / D nav / A beside+mru+sessionbar / A·B composer / A notifications | **closed** throw 回滚/notice；peek；closeEditors；opener 回滚；stale；save/delete/cancel/Stop/Goal/Enqueue false 先 notice；reveal acquire；open-beside / primary bootstrap catch error notice；Goal 断连+history 可点；permission/question roster false；SessionBar rename/delete 与 Sessions 侧栏 delete false notice；notifications acquire throw → [] + error；未关 D16 |

## 工位表（P0 盘点 · 2026-09-08 · 与 `git worktree list` 对照）
| 槽 | 路径 | 分支 | tip | 脏 | stash | 关仓状态 |
|----|------|------|-----|:--|:------|:---------|
| merge | `vscode-WorkTrees/merge` | `loop/merge` | `b1e75b142ac` | `__pycache__` | 0 | parked；compile unused 仍红，**不 push** |
| A | `vscode-WorkTrees/A` | `loop/A` | `b1e75b142ac` | `__pycache__` | 0 | idle |
| B | `vscode-WorkTrees/B` | `loop/B` | `b1e75b142ac` | `__pycache__` | 0 | idle |
| C | `vscode-WorkTrees/C` | `loop/C` | `b1e75b142ac` | 未提交 `dev/loop` + `__pycache__` | 0 | idle；勿 add `dev/loop` |
| D | `vscode-WorkTrees/D` | `loop/D` | `b1e75b142ac` | `__pycache__` | 0 | idle |
| edit | `Projects/Agents/vscode` | `agent-ide` | `a40a95d1e85`+ | `dev/loop` + 过期 progress | 0 | 人类工位；请自行对齐 `b1e75b142ac` |
## Next（Blockers：无）
| 项 | 指针 |
|:---|:-----|
