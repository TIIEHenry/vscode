...
phase: M7
updated: 2026-09-11
summary: "GitHub Actions 永久关闭（见 workflows/DISABLED.md）。loop：D273–D275 / D277 已 push；本合入 D276；A D279 / B D278 queued；compile-client 待三槽齐。"
---
...
### 进行中（2026-09-11 · 合入 D · A/B queued）
| 槽 | 切片 | 状态 |
|:---|:-----|:-----|
| **A** | catalog-plugins-mcp-runtime-detail | merge-queued；D279 Plugins/Runtime 详情 leftover |
| **B** | pairing-catalog-keep-last | merge-queued；D278 pairing keep-last |
| **C** | — | idle；勿 add `dev/loop` |
| **D** | navigator-read-unknown-leftover | 已进 merge；[D276](deferred-gaps.md) 已闭 |

<details>
<summary>历史切片流水（D45 起，已闭项见上表）</summary>

集成 tip **`0e3b09d3`**（`loop/merge`；已 push；compile-client 0）。Chat 仍被引擎空壳 Create（目录在、`session_meta` 空、回 6）挡住；不要再清 `.sessions` 当主线。见 [D26](deferred-gaps.md)。**U2 未开**。**PRD-008 / PRD-019 不升 `implemented`**。[R8](research-queue.md) **已闭**（[ADR-008](../decisions/008-write-git-apply-hunks-empty.md) 引擎空 `patches` = 成功空操作）。Accept 产品选项 A 见 [sources-accept-empty-success](../plans/sources-accept-empty-success.md)（`draft`；**A1 已落**；**P5 停线**——只批准停线，不批准 A2；A2 须新选定 + 新 Arch-First）。
...
| [D242](deferred-gaps.md)–[D277](deferred-gaps.md) | leftover + pairing | **closed** catalog leftover + Overview Model + Sources no-hook + Activity fetch-fail + 详情二次 throw + Agents/Team UNKNOWN leftover + H4b pairing |
...
|----|------|------|-----|:--|:------|:---------|
| merge | `vscode-WorkTrees/merge` | `loop/merge` | 本 D 合入 | 干净 | 0 | 合入 D276；A/B queued；未 compile |
| A | `vscode-WorkTrees/A` | `loop/A` | `56db32094f7` | 干净 | 0 | merge-queued D279 |
| B | `vscode-WorkTrees/B` | `loop/B` | `c37629f2972` | 干净 | 0 | merge-queued D278 |
| C | `vscode-WorkTrees/C` | `loop/C` | `0e3b09d3f89` | 未提交 `dev/loop` | 0 | idle；勿 add `dev/loop` |
| D | `vscode-WorkTrees/D` | `loop/D` | `161051d4763` | 干净 | 0 | 已进 merge；D276 已闭 |
...
| **loop 切片** | D275/D277 已 push。本合入 D [D276](deferred-gaps.md) 已闭。在途：A D279 Plugins/Runtime 详情、B D278 pairing 清行。勿派 D8/D147 / 勿开 F3·A2 / 勿发明 proto |
