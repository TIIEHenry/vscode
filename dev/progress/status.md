...
phase: M7
updated: 2026-09-09
summary: "D16 / D31 F4 仍开。官方三域 glob 已绿。D44–D90 / D92–D129 / D131 / D133–D142 / D144 已闭。requestResync reject 已吞。A2 仍 blocked。"
---
...
65. **B 槽 `engine-frame-resync-catch`（未关 D16 / D31）**：`requestResync` reject 已 `.catch`；坏帧不落地；无未处理 rejection。[D144](deferred-gaps.md) **已闭**。未 compile。
子 agent 发现的既有代码问题：
| ID | 来源 | 问题 |
|:---|:-----|:-----|
...
| [D140](deferred-gaps.md)–[D142](deferred-gaps.md) | A SessionBar / B Sessions 侧栏断连 New session / D mermaid getExtension reject | **closed** 断连+history New session 画 notice 且不调 `createSession`；mermaid reject 回 `undefined`；未关 D16 |
| [D144](deferred-gaps.md) | B `requestResync` reject | **closed** `.catch`；坏帧不落地；无未处理 rejection；未关 D16 |
