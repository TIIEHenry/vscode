...
/** INV-NO-COPILOT: ChatView / ChatEditor open paths are Agents Window only. */
export function canOpenChatViewOrEditor(isSessionsWindow: boolean): boolean {
...
export class ChatWidgetService extends Disposable implements IChatWidgetService {
	...
	async revealWidget(preserveFocus?: boolean): Promise<IChatWidget | undefined> {
		if (!canOpenChatViewOrEditor(this.environmentService.isSessionsWindow)) {
			return undefined;
		}
	...
	async openSession(sessionResource: URI, target?: typeof ChatViewPaneTarget | PreferredGroup, options?: IChatEditorOptions): Promise<IChatWidget | undefined> {
		if (!canOpenChatViewOrEditor(this.environmentService.isSessionsWindow)) {
			this.logService.trace(`[ChatWidgetService] openSession skipped default window uri=${sessionResource.toString()}`);
			return undefined;
		}
