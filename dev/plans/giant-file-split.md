...
phase: N/A
updated: 2026-09-04
summary: "四份巨型文件按状态机/投影/IO 拆（不改对外契约）；GFS-1 先落 mapper 特征测再搬；GFS-4 在 Desktop 拆且每文件 ≤800 硬上限、G6 只能上游删；sessionCore/sessionView 仿既有 SYNC.md 回同步，禁止手改 vendored"
---
...
| 序 | 切片 | 为什么现在 |
|:--:|:-----|:-----------|
| 1 | **GFS-1 `grpcClient`** | 自有；~1100 行 mapper/codec 与类体已分层；不碰 Actor / lens / fanout。工厂仍从 `grpcClient.js` 出口则**不改** `universeAgentConnectionService`，可与 P 槽 [GC-1b](m7-gap-closeout.md) 并行 |
| 2 | **GFS-2 `conversationTimelineTree`** | 自有；class 边界已画好，是搬文件不是重写。共享类型先落无环模块再搬 renderer，避免 tree ↔ renderer 循环 |
| 3 | **GFS-3 `conversationLens`** | 自有；可维护性收益最大，但门面仍是会话集成枢纽。排在 GFS-2 后。初案门面残留 ~1110，已扩 chrome / dock 切片 |
...
| PIN | `02a2ba350f82d3b087cb664942624f71fa4e0c60`（两份 session-core SYNC.md；本机 Desktop `HEAD` 同值） |
...
| 切片 | 前置 / 后续 | 原因 |
|:-----|:-----------|:-----|
| GFS-1 | 无前置。合入后 [packaging-and-release §12 / §3.1](packaging-and-release.md) 的 `@grpc/grpc-js` import 点清单要**用 `rg` 重新数**（多出 `grpcClientCalls.ts`），不抄旧表 | packaging 静态依赖根检查按 import 点计数 |
| GFS-2 | 无前置。与 [prd-020](prd-020-turn-fixture-bench.md) 共写 `conversationTimelineTree.ts` 附近——prd-020 B1 探针只读 DOM 不改树，可并行；若 prd-020 需加 `data-*` 钩子，走 GFS-2 合入后 | 避免同文件两写者 |
| GFS-3 | **等** [test-baseline-ci](test-baseline-ci.md) 切片 1（D16 账本落地）合入。`conversationLens.test.ts` 当前有账本内的红项，拆前没有绿基线就分不清「拆坏」与「本来红」 | 拆分的机器真值是 it() 不减 + 账本外零新红 |
...
- 本稿 2026-09-05 **签收（`accepted`）**。实施顺序：GFS-1a（特征测）→ GFS-1b ∥ GFS-2 → GFS-3（等 test-baseline 切片 1）→ GFS-4（等 G6 上游删除；Desktop 写者与 G-CORE-1 / G2 串行）。
