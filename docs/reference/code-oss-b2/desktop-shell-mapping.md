...
| Navigator body | `SIDEBAR_PART`（Activity 默认标签 **Files**（同一 Explorer `VIEW_CONTAINER`，`isDefault: true`）+ 非 default 容器 Sessions / Projects / Agents / Team；**不含** Run and Debug / Source Control / Testing / Extensions / Voice Transcripts / Voice Event Stream——六者 `IsSessionsWindowContext` 门闩，Agents Window 可保留；Test Results 与 Debug Console 仍在 Panel） | 保留 Explorer 权威；roster = Files + **Sessions stub roster**（`IConversationStubService` 内存标题列表，点击切 Conversation 当前 stub，View 标题 **New session**，高亮 = `getActiveSessionId()`，与 SessionBar 同步；无 Copilot / `IChatModel`）+ Projects/Agents/Team honest stub（not connected — no engine，无 Copilot / 假行） | `toggleRegion('navigatorBody')` ≙ `setSideBarHidden` + 记宽 |
