...
created: 2026-08-30
updated: 2026-09-07
summary: "延期缺口 SSOT；D16 仍开（切片 3 名单已落，三文件标题未列入；compile/S2 leftover 不闭）；D17 leftover 中 C 槽已修 visualize / T5a / trajectory label / process fold；D42 最大化藏轨迹槽；D22 F3；D24 其余 JSON RPC；D25 引擎 List 真空；D26 引擎建壳回 6；D31 F4 / A2 blocked"
---
...
| D42 | P3 | **Maximize 把整个 `.conversation-timeline` 槽 `display:none`**（`conversationLens.css`），轨迹页与对话树同槽，最大化后切 Trajectory 虚拟列表不铺行 | C 槽 T5a 测已用 `display:block` + `revealRecord` 夹具绕过；不改 CSS / 不占视觉域 | 最大化后切 Trajectory 仍能见行；或 CSS 只藏 `.conversation-lens-timeline` 不藏轨迹 | conversation | open |
...
| `d2abb648c0e` | conversation：Lens visualize 两败 + reveal navigation 三败 + process fold span **已由 C 槽夹具/合同对齐收口**（`out/` visualize 测曾落后源码；T5a ResizeObserver/`this.timeout`/`lens.layout`；最大化见 [D42](#d42)）。pending-on-focus 未动 | leftover | C `lens-visualize-t5a` |
| `d2abb648c0e` | conversation：StatusBar 引擎入口 / H4b 相位文案；SessionsView 种子行；split / side-by-side。**trajectory 角色标签 Permission≠Confirmation 已由 C 对齐为 Permission**（产品座名）。未占 StatusBar / SessionsView / split | baseline | A `test-baseline-slice3` / C `lens-visualize-t5a` |
