...
1. 会话列表、当前会话、发送、权限座位来自引擎诚实枚举与流，而不是 `untitled` / `tour` / stub echo 冒充已接通。
...
| 把 stub 种子会话 id（`untitled`/`tour`/`blank`）写成引擎 session id | 接通后换 UA id |
...
|----|-----------|-------------------------------|------|
| 握手 | `isEngineConnected()` / `setEngineConnected`（测试可写布尔） | `SystemService.Connect` → `ConnectResponse`（`capabilities.methods` / `events`，**无** proto 字段 `supported_methods`；`session_token`；`work_dir`） | stub 无握手、无 token、无 methods 广告。生产连接态必须来自 Connect 成败，禁止 UI 手拨 |
| 会话目录 | `getSessions` / `createSession` / `deleteSession` / `renameSession` | `SessionService.List` / `Create` / `Delete`；标题 `AgentService.Rename` | stub 三颗种子；UA 分页 + `SessionListFilter` + shelve。**无** `SwitchSession` RPC |
...
SystemService / SessionService / AgentService / PermissionService / ToolService / McpService
        ▲  native gRPC（Node / utility / shared）
IUniverseAgentConnection（platform/universeAgent）
        │  进程内 ProxyChannel；renderer 只持 common 契约
...
1. **进程：** 沙箱 renderer 不能直接持 HTTP/2 gRPC。对照 `platform/agentHost`（common 契约 + node 宿主 + electron 装配）。UA 客户端同构：`common` 接口、`node` stub、`electron-browser` 代理。
...
**contrib 职责：** `registerSingleton(IConversationRosterService, <EngineBackedRoster>, …)` **替换实现类**，不换 token。类内：未连接 → 现有 stub 种子（壳 UX）；已连接 → UA `List`/`GetHistory`/`Chat`；断连后 → [§6](#6-断连回退与诚实空)。
...
| 来源 | 键 | vscode 做法 |
|------|----|-------------|
| Connect `ServerCapabilities.methods` + 运行时 probe | UA 已有：`skills` / `mcp` / `plugins` / `globalRules`（以及 memory/token/… 本页不消费） | 握手 methods 命中 → 可标 SUPPORTED 候选；`UNIMPLEMENTED` → UNSUPPORTED。**禁止**把空列表当 SUPPORTED |
...
**Conversation 壳：** `isEngineConnected()` 在接通后变为「UA Connect 成功且通道未死」。切片 5 StatusBar：connected → Engine pane，否则 Connection（page-access B10）。能力三态 **不**用来画会话列表假行。
...
| 表面 | 从未连接 | 已连接后断连 |
|------|----------|----------------|
| Conversation roster / 时间线 | 继续 stub 种子 + stub echo（PRD-003 诚实占位） | **保留**最后一次 UA 快照只读；标题/回合不换成 `untitled`/`tour`；输入可拒发或明确「Engine not connected」；**禁止** stub echo 看起来像引擎回复 |
...
**做什么：** `platform/universeAgent` 连接 + Connect + capability probe + `SessionService.List/Create/Delete` + `GetHistory` 只读投影到 roster；实现类替换 stub；`isEngineConnected` 接真握手。发送可先仍走 stub echo **仅当未连接**；连接成功后发送可先 fail-closed（拒绝 + 诚实句）直到 M6-B。

**不做什么：** Engine catalog UI、StatusBar 改开 Engine pane、轨迹 fold、Device pairing 完整流（loopback 无 pairing 即可；需要 pairing 则 Connection pane 诚实待配对，不静默跳过）。

**验证：** platform 单测 mock channel：Connect 成功 → `isEngineConnected===true`；UNIMPLEMENTED skills → `UNSUPPORTED` 而非空列表；断连不回填 stub 种子到 UA id。`valid-layers-check`：`contrib/conversation` 不得 import `platform/universeAgent/node`。

### M6-B — page-access 切片 5

父方案原文：adapter 同 token 已在 A 完成；本刀只做 UI 合同。

- StatusBar B10：connected → `openEnginePreferences`，否则 Connection。
- roster listed id 与 SessionBar 标题一致；`getActiveSessionId` 单源。
- Send **不**调用 `IChatService.sendRequest`（负向 mock）；改为 `AgentService.Chat`。
- Dock 控件集 3b 按各页分阶段，本刀不接新引擎实现。
- SessionBar 去 SelectBox 仍 Deferred。

**验证：** 父方案切片 5 Tests + 负向 send 测。
...
| 断连回 stub 种子造成「会话消失又冒出 Untitled」 | §6；测：UA id 在 `onDidChangeEngineConnection(false)` 后仍在 `getSessions()` |
