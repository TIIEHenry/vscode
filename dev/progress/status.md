...
- **槽 B / `loop/B`：** Engine Tools 选中行消费已进 catalog 的 `getToolInfo?`：只读详情（description / category / aliases / destructive / requiresPermission）；有 `inputSchemaJson` 只提示「引擎提供、此处不可编」，**不**画 schema 编辑器。无 hook 仍 list-only。测：选中转发 + 无 hook 不画详情。此前 ToolInfo catalog + node unary。
...
| B | `vscode-WorkTrees/B` | `loop/B` | Engine Tools 选中行读 ToolInfo 只读详情；无 hook 仍 list-only |
