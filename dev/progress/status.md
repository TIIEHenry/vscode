...
phase: M7
updated: 2026-09-16
summary: "merge compile-client 0。A 槽 listTools/listSkills bytes；D24 仍开（MCP/plugins/clipboard JSON）。不是 leftover/pills 完成。R9/D405 仍开"
---
...
### 进行中（2026-09-16 · A `d24-listtools-listskills-bytes`；D24 仍开；不是 leftover/pills 完成）
| 槽 | 切片 | 状态 |
|:---|:-----|:---------|
| **A** | `d24-listtools-listskills-bytes` | `listTools`/`listSkills` unary bytes（J 仓 proto 字段号）；MCP/plugins/clipboard 仍 JSON。[D24](deferred-gaps.md) **仍开**。不是 leftover/pills 完成 |
...
| [D405](deferred-gaps.md)–[D477](deferred-gaps.md) | A/D 本波 | D405 手测仍开；D471–D477 已闭；**D24 仍开**（listTools/listSkills 已 bytes；MCP/plugins/clipboard 仍 JSON）；不得宣称 leftover/pills 完成 |
...
| **loop 切片** | A `listTools`/`listSkills` unary bytes（J 仓字段号）。[D24](deferred-gaps.md) **仍开**（MCP / plugins / clipboard / respondQuestion 仍 JSON）。R9 仍开。**D405** 手测仍开。不关 D8/D16/D147。未升 PRD-008。不得宣称 leftover / pills 完成。 |
