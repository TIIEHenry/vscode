...
1. **集成 tip** 以 merge 本关仓提交为准（切片 3 A=`c01e16fad84` → merge `44d65d1da44`；unit-custom XML A=`a167f755fb4`；DiffReview B=`ebe958ab148`）。`npm run compile` 仍基线 unused 红则 **不 push**。GFS >800 不拆。D22/F3 已撤回。不跑 F4 / 不实施 A2。
2. 本波字母槽已进 merge：
