...
- **槽 merge：** 合入 A 的 Inbox AutoDrive 接通 / 断连缓存诚实空、B 的时间线 `deleteTurn` 转发 `AgentService.DeleteMessage`、C 的时间线 `updateUserTurnText` / `turnEdit` 转发 `AgentService.EditMessage`（空 turnId / 空正文不发；未接通仍本地改）、D 的 `AgentService.RespondQuestion` catalog（node unary；问题座仍 Chat 臂）。ToolInfo / Respond / Enqueue / Kill / MessageQueue 已在 tip。
...
| 槽 | 路径 | 分支 | 状态 |
|----|------|------|------|
| merge | `vscode-WorkTrees/merge` | `loop/merge` | AutoDrive 诚实空 + deleteTurn DeleteMessage + turnEdit EditMessage + RespondQuestion catalog + ToolInfo + Respond + Enqueue + Kill / MessageQueue |
