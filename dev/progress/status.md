...
phase: M7
updated: 2026-09-04
summary: "SessionService.Info 已进 gRPC catalog（node unary getSessionInfo；空 sessionId 原样上线；无 Conversation UI）；接通后 Conversation 动作 createSnapshot 转发 AgentService.CreateSnapshot；Conversation SessionBar Snapshots 只读 listSnapshots；CreateSnapshot / RestoreSnapshot / DeleteSnapshot 已进 catalog；respondClientTool / RespondQuestion / DeleteMessage / EditMessage / ToolInfo / Respond / CancelToolCall / Kill / MessageQueue 已转发；Composer 发送仍走 submitInput"
---
...
- **槽 merge：** 合入 A 的 `RestoreSnapshot` catalog、B 的 SessionBar Snapshots 只读 `listSnapshots`、C 的 `DeleteSnapshot` catalog、D 的接通后 `createSnapshot` 转发。tip 另有 SendClientToolResponse / RespondQuestion / EditMessage / AutoDrive 诚实空 / DeleteMessage / ToolInfo / Respond / Enqueue / Kill / MessageQueue。
- **槽 A / `loop/A`：** `AgentService.RestoreSnapshot` catalog + node unary；空 id 原样上线。**不改** SessionBar History / Snapshots overlay。
- **槽 B / `loop/B`：** SessionBar Snapshots 只读 `listSnapshots`；History 仍回合索引。
- **槽 C / `loop/C`：** `AgentService.DeleteSnapshot` catalog + node unary；空 id 原样上线。**不改** Snapshots overlay。
