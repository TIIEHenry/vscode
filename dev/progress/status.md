...
58. **B 槽 `session-window-primary-bootstrap-catch`（未关 D16 / D88 / D89）**：`ensurePrimaryWindow` 先 `ensureLeaf` 再提交 `primarySessionKey`；throw 回滚半应用 leaf，void 调用点未改。[D90](deferred-gaps.md) **已闭**。
子 agent 发现的既有代码问题：
| ID | 来源 | 问题 |
|:---|:-----|:-----|
...
| [D90](deferred-gaps.md) | B 槽 `session-window-primary-bootstrap-catch` | **closed** `ensurePrimaryWindow` throw 已回滚半应用 primary；未关 D16 / D88 / D89 |
...
| B | `vscode-WorkTrees/B` | `loop/B` | `c6c9a8c7d37` | D90 未提交 | 0 | D90 已落未关仓；未关 D16 |
