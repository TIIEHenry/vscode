...
| `AgentService` | `ListSnapshots` | `IUniverseAgentConnection.listSnapshots?`（unary） | **已进** `UniverseAgentGrpcServices.Agent` + node snake_case `session_id`。响应 `snapshots[]`（`id`/`session_id`/`title`/`description`/`created_at`/`turn_count`/`token_count`/`model_id`/`is_auto`）。合同可选。本刀只进 catalog + node 传输；**不**改 SessionBar History（仍回合索引，不把 snapshot 列表冒充历史）。Create / Delete snapshot 未进 |
| `AgentService` | `RestoreSnapshot` | `IUniverseAgentConnection.restoreSnapshot?`（unary） | **已进** `UniverseAgentGrpcServices.Agent` + node snake_case `session_id`/`snapshot_id`。响应 `success`/`error_message`。合同可选。空 `sessionId` / 空 `snapshotId` 原样上线。本刀只进 catalog + node 传输；**不**改 SessionBar History / roster。Create / Delete snapshot 未进 |
