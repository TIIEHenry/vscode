...
phase: M7
updated: 2026-09-04
summary: "D16 Lens 叶宽夹具 + 共享 ResizeObserver harness；Rename/Cancel catalog；roster 接通后转发 Rename；Overview 隐藏 Provider 行"
---
...
| SessionEventStream close | 三路宿主 `onClosed` 已齐：SessionEventStream → `streamClosed`；Chat → `chatStreamDown`；ContinueGeneration 只拆句柄。connection 测现覆盖 Chat / EventStream / Continue 三路 close gate（remote 一次、dispose 静音）。**传输已进** `ContinueGeneration`、`Rename`、`Cancel`；roster 接通后已转发 Rename unary（Inbox Stop 仍本地） |
