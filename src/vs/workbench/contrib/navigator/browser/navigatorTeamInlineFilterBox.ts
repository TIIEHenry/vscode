...
import './media/navigatorTeamInlineFilter.css';
...
export class NavigatorTeamInlineFilterBox extends Disposable {
	...
	constructor(parent: HTMLElement, placeholder: string, ariaLabel: string) {
		...
		this.element = dom.append(parent, $('.navigator-team-inline-filter'));
		const icon = dom.append(this.element, $('.navigator-team-inline-filter-icon'));
		icon.appendChild(renderIcon(Codicon.search));

		this.input = dom.append(this.element, $('input.navigator-team-inline-filter-input')) as HTMLInputElement;
