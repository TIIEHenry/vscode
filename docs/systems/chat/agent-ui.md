...
> **产品 Conversation 系统正文**（Part、session 窗口、透镜、Composer、stub 契约）见 [docs/systems/conversation/](../conversation/INDEX.md)；本页只保留 Copilot Chat 宿主边界与 donor 对照，§3 的 Conversation 段落是指针摘要。  
...
- 默认 Code 窗口在 **M1 亦已落地** 同构拓扑（`ConversationPart` 中心透镜 + End Editor/Sources Files + titlebar 四钮 D7）；**M5 切片 1–3** 已加固默认窗 Chat 路由与 roster 导航、清掉 workbench→sessions 的 `IAgentHostMcpServer` 依赖。启动演示（D4）已于 rerun-2230 全 PASS，EH 探针（D5）已于 wave3 闭合；panel / terminal 次级探针仍记 [D9](../../../dev/progress/deferred-gaps.md)。
...
**StatusBar（默认 Code 窗口）：** `contrib/conversation` 注册右对齐产品芯片（无 Copilot / 无额度 / 无 Sign In）：`status.conversation.session`（`IConversationRosterService` 当前会话标题，无标题 **No session**；点击 `workbench.action.showConversationPart`）、`status.conversation.engine`（文案由 `IUniverseAgentConnection.getConnectionPhase()` + pairing-pending 映射，[connection-hub H4b](../../../dev/plans/connection-hub-client.md) / `getConnectionPhaseStatusBarText` in `conversationSessionStatus.ts`：disconnected / connecting / connected / failed 各态；**pairing-pending 仍算未 connected**，文案可仍为 **Engine not connected**）、`status.conversation.model`（**No model**，**仅当** `CONVERSATION_PART` 隐藏时注册；座位可见时整槽省略，Dock 为 model owner，UI-INV-14）。**Engine 芯片 command（page-access 切片 5 / B10）：** `getEngineStatusCommandId(phase, pairingPending)` / `isConversationEngineLive` 真 → `workbench.action.openEnginePreferences`（Engine pane）；否则（含 `pairingPending`）→ `workbench.action.openConnectionPreferences`（Connection/SAS；`onDidShow` 落到配对区并 focus）。roster 布尔不是路由源。**同一 B10 路由与 H4b 文案**亦用于阅读列 `ConversationIdentityStrip` 引擎 chip（PreFirst 居中区 / Active 阅读列顶；订阅 `onDidChangeConnection` + `onDidChangeEngineConnection`）。session / model 点击仍走 `showConversationPart`；不打开 ChatEditor 或 Aux Chat。无 session-usage / turns / tok/s 芯片（无权威则省略槽位）。
...
- [Conversation 系统](../conversation/INDEX.md) — 产品中心的系统正文（Part、session 窗口、透镜、Composer、stub 契约、命令）
- [Sources 系统](../sources/INDEX.md) — End 列下格 Files / Changes / Review
...
- 空会话 / 输入面（已实施）：[conversation-empty-hero](../../../dev/plans/conversation-empty-hero.md)（PRD-015）
