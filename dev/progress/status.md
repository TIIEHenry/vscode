...
phase: M7
updated: 2026-09-04
summary: "槽 C：PermissionService.Respond 进 gRPC catalog + node unary；权限回执仍走 Chat 臂；Kill / CancelToolCall / ToolInfo / Fork / Inbox Goal / MessageQueue catalog 已在 tip"
---
...
| SessionEventStream close | 三路宿主 `onClosed` 已齐：SessionEventStream → `streamClosed`；Chat → `chatStreamDown`；ContinueGeneration 只拆句柄。connection 测现覆盖 Chat / EventStream / Continue 三路 close gate（remote 一次、dispose 静音）。**传输已进** `ContinueGeneration`、`Rename`、`Cancel`、`CancelToolCall`、`SessionService.Create`、`PermissionService.SetSessionGoal` / `CancelSessionGoal` / **Respond**、**MessageQueue 族七 unary**、`AgentService.Fork`、`AgentService.Kill`、`ToolService.ToolInfo`；roster 接通后已转发 Create / Rename / Cancel / SetSessionGoal / **Fork**（Create 用引擎 id；Inbox Stop 仅 connected+streaming；Inbox Goal 仅 connected 启用；未指定 agent / Fork parent 用末条 streaming 否则 `root`；Fork 不造本地 catalog id）。队列 UI 仍 fixture；Kill / Respond **未**转发；未接通 Fork tab 仍 `registerForkChat`；Engine Tools 仍 list-only；**时间线尚未转发 CancelToolCall** |
