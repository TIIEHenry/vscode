...
| [connection-hub-client.md](connection-hub-client.md) | `draft` | UniverseAgent Connection Hub 接入分析：IDE 为 Client 设备；`platform/universeAgent/node` 增 Hub 解析 + Device Grant 拨号臂（Desktop Main 模块为 donor）；v1 中继 + DirectAddress、直连 v2；切片 H0–H6；**规则 16 未跑** |
