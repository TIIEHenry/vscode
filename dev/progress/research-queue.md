...
created: 2026-08-30
updated: 2026-09-07
summary: "待研究队列 SSOT；R1–R7 已闭；R8 仍开：ADR-008 已写本仓不能裁定空 argv/patches 引擎语义；剩余问题见该 ADR。"
---
...
| R8 | `WriteGitApplyHunks` 空 `argv` / `patches` 语义 | Review Accept 按合同不发明 hunk，现原样送空列表；若引擎把空请求当成功 no-op，仍可能假 Accept | **本仓已查**：无 `git_service.proto`；协议面 / 类型注释只保证空列表原样上线，**不能**裁定 apply-all / 拒写 / 成功 no-op。书面结论 [ADR-008](../decisions/008-write-git-apply-hunks-empty.md) + [engine-protocol-surface](../../docs/reference/universe-agent/engine-protocol-surface.md) `WriteGitApplyHunks` 行。**剩余问题**：引擎仓 proto 或实现（或活引擎 unary + 工作树对照）对空 `argv` 且空 `patches` 回哪一种 | 引擎仓/活引擎证据后才能闭本行；Sources Accept 是否传 path 仍等本问题 | open |
