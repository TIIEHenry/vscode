...
- **槽 B / `loop/B`：** D16 其余 Lens 夹具：抽出 `conversationLensLayoutHarness`（ResizeObserver loop 拦截 + layout flush），接到 reveal / trajectory / trajectoryUi。此前进口界扫 `src/`；`openChatStream` close-gate 合同测（remote 一次、dispose 静音）。
- **槽 C / `loop/C`：** D16 leftover：`conversationLens.test.ts` 改走共享 `conversationLensLayoutHarness`（ResizeObserver loop 拦截 + layout flush），after-each 与 reveal/trajectory 同式。不改 Inbox Stop / roster。此前 ContinueGeneration + Rename + Cancel 进 gRPC catalog。
...
| C | `vscode-WorkTrees/C` | `loop/C` | D16 conversationLens.test.ts 接入共享 layout harness |
...
| V | D16 `conversationLens.test.ts` 已接共享 harness；剩 DOM/codicon 断言债与 identity 本地 listener；D17 与产品验证 |
