...
phase: M7
updated: 2026-09-04
summary: "MessageQueue 族进 gRPC catalog（Enqueue/Pause/Resume/Clear/Hold/Release/Edit）；roster 仍 fixture；Create/Rename/Cancel 已转发"
---
...
| SessionEventStream close | 三路宿主 `onClosed` 已齐：SessionEventStream → `streamClosed`；Chat → `chatStreamDown`；ContinueGeneration 只拆句柄。connection 测现覆盖 Chat / EventStream / Continue 三路 close gate（remote 一次、dispose 静音）。**传输已进** `ContinueGeneration`、`Rename`、`Cancel`、`SessionService.Create`、**MessageQueue 族七 unary**；roster 接通后已转发 Create / Rename / Cancel（Create 用引擎 id；Inbox Stop 仅 connected+streaming 启用；未指定 agent 用末条 streaming 否则 `root`）。队列 UI 仍 fixture |
