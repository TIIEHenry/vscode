...
phase: M7
updated: 2026-09-04
summary: "Engine Tools 选中行拉 ToolService.ToolInfo 只读详情（无 schema 编辑器）；CancelToolCall 空 agent 用末条 streaming 否则 root；Kill roster 已转发；MessageQueue 五操作 + Edit 接通后 roster 转发（无 GetQueue 显示诚实空；Enqueue 仍仅传输）；PermissionService.Respond catalog 已进、权限回执仍走 Chat 臂；Fork / Goal / Create / Rename / Cancel 已转发"
---
...
- **槽 merge：** 接通后时间线 Cancel Tool → `CancelToolCall`：保留 A 的额外 fold 测 + B 的末条 streaming 否则 `root`。合入 C 的 `PermissionService.Respond` catalog、D 的 MessageQueue roster 转发、A 的 Engine Tools ToolInfo 只读详情与 Kill roster。
...
- **槽 D / `loop/D`：** 接通后 roster 转发已进 catalog 的 MessageQueue 五操作 + `EditQueueItem`（未知 session / 空 item / 空正文 / 断连缓存不发）。`getMessageQueueState` 接通后诚实空（无 GetQueue，不把 fixture 冒充引擎队列）；`EnqueueQueueItem` 仍仅传输。不改 CancelToolCall 时间线。此前 Kill catalog（roster 未转发）。
...
| SessionEventStream close | 三路宿主 `onClosed` 已齐：SessionEventStream → `streamClosed`；Chat → `chatStreamDown`；ContinueGeneration 只拆句柄。connection 测现覆盖 Chat / EventStream / Continue 三路 close gate（remote 一次、dispose 静音）。**传输已进** `ContinueGeneration`、`Rename`、`Cancel`、`CancelToolCall`、`SessionService.Create`、`PermissionService.SetSessionGoal` / `CancelSessionGoal` / **Respond**、**MessageQueue 族七 unary**、`AgentService.Fork`、`AgentService.Kill`、`ToolService.ToolInfo`；roster 接通后已转发 Create / Rename / Cancel / SetSessionGoal / **Fork** / **Kill** / **CancelToolCall** / **MessageQueue 五操作 + Edit**（Create 用引擎 id；Inbox Stop 仅 connected+streaming；Inbox Goal 仅 connected 启用；未指定 agent / Fork parent / CancelToolCall agent 用末条 streaming 否则 `root`；Kill 空 agent **不**默认 `root`；Fork 不造本地 catalog id；队列无 GetQueue 显示空，Enqueue 仍仅传输）。Respond **未**转发（权限回执仍走 Chat 臂）；未接通 Fork tab 仍 `registerForkChat`；Engine Tools 选中行拉 `getToolInfo` 只读详情（不画 schema 编辑器） |
