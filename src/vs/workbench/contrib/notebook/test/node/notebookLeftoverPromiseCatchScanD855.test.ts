...
const d855Calls: Array<[string, string, number]> = [
	[STATUS_REL, updateCall, 8],
];

suite('leftover remaining unused after D844 leftover remaining unused notebook leftover remaining unused Promise fire-and-forget catch scan (D855)', () => {
	...
	test('leftover remaining unused after D844 research already-double still has four or more legal unused leftover sites so this knife stayed on notebook leftover remaining unused _update', () => {
		...
		let sites = 0;
		for (const [, call, count] of d855Calls) {
			const wrapped = countIncludes(status, `${call}${doubleCatch}`);
		...
		assert.ok(sites <= 8);
		assert.ok(!findModel.includes('D855'));
		assert.ok(!status.includes('D855'));
	});

	test('this knife covers eight leftover Promise double-chain sites after leftover remaining unused stayed on notebook leftover remaining unused _update', () => {
		let sites = 0;
		const seen = new Map<string, string>();
		for (const [rel, call, count] of d855Calls) {
		...
		assert.ok(sites >= 4);
		assert.ok(sites <= 8);
		assert.strictEqual(countDoubleChains(seen.get(STATUS_REL) ?? ''), 8);
	});

	test('notebook leftover remaining unused async this.foo() FOF leftover void promises are Promise/async + double-chain', () => {
		const source = fs.readFileSync(resolveSource(STATUS_REL), 'utf8');
		assertPromiseSignature(source, 'private async _update() {');
		assert.ok(source.includes("import { onUnexpectedError } from '../../../../../../base/common/errors.js';"));
		assertWrapped(source, updateCall);
		assert.strictEqual(countIncludes(source, `${updateCall}${doubleCatch}`), 8);
		assert.ok(!source.includes('\t\tthis._update();\n'));
		assert.ok(!source.includes('\t\t\t\tthis._update();\n'));
		assert.ok(!source.includes('\t\t\t\t\t\tthis._update();\n'));
		assert.ok(!source.includes('() => this._update())'));
		assert.ok(!source.includes('() => this._update(),'));
		assert.ok(!source.includes(`await this._update()${doubleCatch}`));
		assert.ok(!source.includes('await this._update();'));
	});
	...
	test('opener / Action2.run / assigned then / two-arg then / returned Promise / already-double / Resolve / Pty / Connect / Watch / D145 stay skipped', () => {
		...
		assert.ok(!source.includes('return this._update()'));
		assert.ok(!source.includes('await this._update()'));
		...
		assert.ok(!source.includes('D855'));
	});

	test('locked leftover remaining stay leftover remaining unused; D844/D847/D843/D841/D839/D840 already-double stay already-double; this knife did not overflow into sibling leftover modules', () => {
		const source = fs.readFileSync(resolveSource(STATUS_REL), 'utf8');
		const findModel = fs.readFileSync(resolveSource(FIND_MODEL_REL), 'utf8');
		...
		for (const [rel, file] of [
			...
			assert.ok(!file.includes('D855'), `${rel} should stay off this knife`);
		}
		assert.ok(!source.includes('D855'));
		...
		assert.ok(!fs.existsSync(path.join(process.cwd(), 'src/vs/workbench/contrib/searchEditor/test/node/searchEditorLeftoverPromiseCatchScanD855.test.ts')));
		assert.ok(!fs.existsSync(path.join(process.cwd(), 'src/vs/workbench/contrib/bulkEdit/test/node/bulkEditLeftoverPromiseCatchScanD855.test.ts')));
		assert.ok(!fs.existsSync(path.join(process.cwd(), 'src/vs/workbench/contrib/outline/test/node/outlineLeftoverPromiseCatchScanD855.test.ts')));
		assert.ok(!fs.existsSync(path.join(process.cwd(), 'src/vs/workbench/contrib/markers/test/node/markersLeftoverPromiseCatchScanD855.test.ts')));
		assert.ok(!fs.existsSync(path.join(process.cwd(), 'src/vs/workbench/contrib/scm/test/node/scmLeftoverPromiseCatchScanD855.test.ts')));
		assert.ok(!fs.existsSync(path.join(process.cwd(), 'src/vs/workbench/contrib/files/test/node/filesLeftoverPromiseCatchScanD855.test.ts')));
		assert.ok(!fs.existsSync(path.join(process.cwd(), 'src/vs/workbench/contrib/terminal/test/node/terminalLeftoverPromiseCatchScanD855.test.ts')));
		assert.ok(!fs.existsSync(path.join(process.cwd(), 'src/vs/workbench/contrib/search/test/node/searchLeftoverPromiseCatchScanD855.test.ts')));
		assert.ok(!fs.existsSync(path.join(process.cwd(), 'src/vs/workbench/contrib/comments/test/node/commentsLeftoverPromiseCatchScanD855.test.ts')));
		assert.ok(!fs.existsSync(path.join(process.cwd(), 'src/vs/workbench/contrib/chat/test/node/chatSetupLeftoverPromiseCatchScanD855.test.ts')));
		assert.ok(!fs.existsSync(path.join(process.cwd(), 'src/vs/workbench/contrib/debug/test/node/debugLeftoverPromiseCatchScanD855.test.ts')));
	});
});
