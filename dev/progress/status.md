...
50. **B 槽 `channel-client-hydrate-catch`（未关 D16 / 未发明 proto·GetQueue·heartbeat client_id）**：`UniverseAgentConnectionChannelClient` `hydrate` / `refreshPhaseAndNotify` 包 try/catch；IPC reject 保留已写缓存、不 fire 半应用 phase。测锁无未处理 rejection。[D81](deferred-gaps.md) **已闭**。未 compile。
