...
created: 2026-08-30
updated: 2026-09-04
summary: "延期缺口 SSOT；D16 进口界测已改扫 src/；D19/D21 已闭；D22 F3 lease；D15 欠 W1；不冻结 UI 主线"
---
...
| D16 | P2 | **conversation 单测基线红**：`conversationLens.test.ts` 11 个失败（DOM 高度 0 / codicon 选择器 / 种子会话数 `1 !== 2` 等）、`conversationIdentityStrip.test.ts` 1 个、`conversationStubService.test.ts` 3 个（假定种子会话为空，实际 `untitled` 有 7 条 fixture），在 **不含** stream-timeline S1 改动的 HEAD `0649602d` 上同样失败。**进口界空扫假绿已收 @ 2026-09-04**（四份 `*ImportBoundaries.test.ts` 改扫 `src/` + `>=80` 计数，见 `conversationImportBoundaryScan.ts`） | 2026-09-02 S1 实施时发现；已顺手修两处启动级问题（Lens 夹具缺 `registerPart`/`isVisible` → `TestLayoutService`；`getVisibleTimelineIndices` 对 `lastVisibleElement` 越界防御），剩余为断言级过时 | 逐条分类：过时断言改测 / 产品 bug 修码；stream-timeline S2 退出条件「Lens 全绿」以本行闭合为前提，或改为「S2 不新增失败」。进口界假绿不再挡本行 | M6 / conversation | open |
