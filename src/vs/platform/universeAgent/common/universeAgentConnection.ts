...
export interface IUniverseAgentConnection {
	...
	/**
	 * RemoteAgentService.ListNodes unary. Optional so Web / tests can omit
	 * it. Catalog + node transport only this slice; empty `filterStatus` /
	 * `filterTags` (incl. empty strings) are sent as-is. Empty `id` /
	 * `name` / `description` / `status` / `endpoint` / `tags` mapped
	 * as-is. `lastHeartbeatAt` / `total` / `onlineCount` 0 mapped as-is.
	 * Proto fields only (`ListNodesRequest` / `ListNodesResponse` +
	 * `RemoteAgentInfo` / `Capabilities` / `ModelInfo` / `LoadMetrics`).
	 * No Conversation roster / UI / Engine Preferences / Composer.
	...
	/**
	 * RemoteAgentService.SetMaintenance unary. Optional so Web / tests
	 * can omit it. Catalog + node transport only this slice; empty
	 * `nodeId` is sent as-is. `success` false mapped as-is. Proto fields
	 * only (`SetMaintenanceRequest` / `SetMaintenanceResponse`). No
	 * Conversation roster / UI / Engine Preferences / Composer.
	 * ≠ ListNodes / GetNode / CheckConnection / ExitMaintenance /
	 ...
	 * Empty `id` / `name` / `description` / `tags` / `sessionLifecycle`
	 * mapped as-is. `enabled` false mapped as-is. Nested empty ids /
	 * 0 / false mapped as-is. Proto fields only
	 * (`ListRemoteAgentConfigsRequest` /
	 * `ListRemoteAgentConfigsResponse` + `RemoteAgentConfig` /
	 * `Endpoint` / `AuthConfig` / `PermissionDelegate` /
	 * `WhitelistEntry` / `ArgCondition` / `PermissionBudget` /
	 * `HealthCheckConfig`). No Conversation roster / UI / Engine
	 * Preferences / Composer.
	...
	/**
	 * RemoteAgentService.ResetError unary. Optional so Web / tests
	 * can omit it. Catalog + node transport only this slice; empty
	 * `nodeId` is sent as-is. `success` false mapped as-is. Proto fields
	 * only (`ResetErrorRequest` / `ResetErrorResponse`). No
	 * Conversation roster / UI / Engine Preferences / Composer.
	...
	/**
	 * RemoteAgentService.DestroyRemoteSession unary. Optional so Web /
	 * tests can omit it. Catalog + node transport only this slice; empty
	 * `callId` is sent as-is. `success` false mapped as-is. Empty
	 * `message` mapped as-is. Proto fields only
	 * (`DestroyRemoteSessionRequest` / `DestroyRemoteSessionResponse`).
	 * No Conversation roster / UI / Engine Preferences / Composer.
	...
	/**
	 * RemoteAgentService.GetRemoteSessionStatus unary. Optional so Web /
	 * tests can omit it. Catalog + node transport only this slice; empty
	 * `callId` is sent as-is. Empty `status` / `callId` / `progress`
	 * mapped as-is. `elapsedMs` / `expiresAt` 0 mapped as-is. Empty
	 * `pendingPermissions` / `pendingQuestions` (incl. empty nested
	 * strings) mapped as-is. Proto fields only
	 * (`GetRemoteSessionStatusRequest` /
	 * `GetRemoteSessionStatusResponse` + `RemotePendingPermission` /
	 * `RemotePendingQuestion`). No Conversation roster / UI / Engine
	 * Preferences / Composer.
	 * ≠ CreateRemoteSession / DestroyRemoteSession /
	 * GetRemoteSessionHistory / ResumeRemoteSession /
	 ...
	 * DeviceService / SessionService / AgentService.Status.
	 */
	...
	/**
	 * TriggerService.UpsertTrigger unary. Optional so Web / tests can omit
	 * it. Catalog + node transport only this slice; empty `scope` /
	 * `scopeId` are sent as-is. Empty `triggerId` / `name` / `type` /
	 * `promptTemplate` / `pauseReason` / `cronExpression` sent and mapped
	 * as-is. `enabled` false sent and mapped as-is. `intervalMs` /
	 * `runAtEpochMs` 0 sent and mapped as-is. Proto fields only
	 * (`UpsertTriggerRequest` / `UpsertTriggerResponse` + `TriggerDto` /
	 * `DeliveryTargetDto`). No Conversation roster / UI / Engine
	 * Preferences / Composer.
	...
	/**
	 * ClipboardService.List unary. Optional so Web / tests can omit it.
	 * Catalog + node transport only this slice; empty `sessionId` is sent
	 * as-is. Proto fields only (`ClipboardListRequest` /
	 * `ClipboardListResponse` + `ClipboardEntrySummary`: `clip_id` /
	 * `label` / `type` / `created_by` / `created_at`). Empty `clipId` /
	 * `label` / `createdBy` mapped as-is. `createdAt` 0 mapped as-is.
	 * `type` uses typed `ClipboardEntryType` wire. No Conversation
	 * roster / UI / Engine Preferences / Composer.
