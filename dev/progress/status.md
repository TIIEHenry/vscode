...
50. **D 槽 `navigator-reveal-throw-notice`（未关 D16 / 未关 D21 / 未碰 lens / channel client / 未发明 GetQueue）**：`revealNavigatorAgentInConversation` await 路径 try/catch；`openSubAgent` throw 画 `INotificationService.error(getErrorMessage)`；void 调用点未改。测锁 throw → notification error、无未处理 rejection。[D82](deferred-gaps.md) **已闭**。未 compile。
