...
created: 2026-08-30
updated: 2026-09-04
summary: "延期缺口 SSOT；D16 stub deleteTurn 过时断言已隔离；D19/D21 已闭；D22 F3；D15 欠 W1"
---
...
| D16 | P2 | **conversation 单测基线红**：`conversationLens.test.ts` 仍有 DOM 高度 0 / codicon 选择器等失败；`conversationIdentityStrip.test.ts` 1 个。`conversationStubService.test.ts` 里假定 `untitled` 为空的 `deleteTurn` 断言已改走 `createSession()`（`untitled` 仍是 7 条 fixture）。`conversationImportBoundaries.test.ts` 假绿已由扫 `src/` 收口 | 2026-09-02 S1 实施时发现；启动级夹具已修；本刀只收 stub 服务过时计数，不考古 Lens 11 条 | Lens / identity 逐条分类：过时断言改测 / 产品 bug 修码；stream-timeline S2 退出条件「Lens 全绿」以本行闭合为前提，或改为「S2 不新增失败」 | M6 / conversation | open |
