...
| 槽 | 切片 | 状态 |
|:---|:-----|:---------|
| **A/D** | D24 question/client-tool + D481 | 已合；[D24](deferred-gaps.md) **仍开**。[D481](deferred-gaps.md) **closed**。不是 leftover/pills 完成 |
| **F** | D24 Memory unary wire | `grpcMemoryUnaryWire` + test；未接线 grpcClient；跳过 Rebuild；score double 未读。[D24](deferred-gaps.md) **仍开**。不是 leftover/pills 完成 |
| **B/C/E** | — | B 脏 worktree-pool 勿 `-B`；C 脏 `dev/loop` gitlink；E leftover `blocked` |
...
| [D405](deferred-gaps.md)–[D481](deferred-gaps.md) | A/D 本波 | D405 手测仍开；D471–D481 已闭；**D24 仍开**（Memory grpcClient 仍 JSON；F unary wire 未接线）；不得宣称 leftover/pills 完成 |
...
| B/C | `vscode-WorkTrees/B`·`C` | `loop/B`·`C` | 见右 | 脏 | 0 | B worktree-pool 勿 `-B`；C `dev/loop` gitlink 试 ff |
...
| **loop 切片** | A question/client-tool 已 bytes。F Memory unary wire 未接线 grpcClient。[D24](deferred-gaps.md) **仍开**。[D481](deferred-gaps.md) **closed**。[D487](deferred-gaps.md) score double 未读。R9 / **D405** 仍开。不关 D8/D16/D147。未升 PRD-008。不得宣称 leftover / pills 完成。 |
