...
phase: M7
updated: 2026-09-16
summary: "H 槽 Agent.Prune wire-only 未接线。D24 仍开。关仓 prefs bytes、五条 Agent wire 与 D550–D551 双链。不是 leftover/pills 完成。"
---
...
| **H** | Agent.Prune wire-only；未接线；D24 仍开 |
| **edit** | ff-only 失败（分歧 `ff278772b85`）；勿 reset |

子 agent 发现：
| ID | 问题 |
|:---|:-----|
| [D24](deferred-gaps.md) | **仍开**：Clear/Get·SetModelPreferences **已** bytes；SwitchWorkDir/Compact/Todo/Usage/Status/Prune wire 未接线；ResolveTurn/Connect/SaveSkillContent/Watch·Rebuild 仍 JSON |
...
| 项 | 指针 |
|:---|:-----|
| **loop** | D24 仍开。H 本刀 Prune wire 未接线。A 下一刀接 SwitchWorkDir/Compact/Todo/Usage/Status/Prune。SaveSkillContent 无 RPC。Connect/Watch 跳过。不得宣称 leftover/pills 完成 |
