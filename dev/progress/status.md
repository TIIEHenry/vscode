...
- **槽 B / `loop/B`：** 接通后时间线 process-fold 执行中工具行转发已进 catalog 的 `AgentService.CancelToolCall`（`toolCallId` ≡ 行 id；未指定 agent 用末条 streaming 否则 `root`；空 id / 断连 / 无 hook 不发）。不改 Inbox Stop（仍只转 `Cancel`）。测：roster 转发 / 断连跳过 / 执行中行 Cancel。此前 Engine Tools 选中行读 ToolInfo 只读详情。
...
| B | `vscode-WorkTrees/B` | `loop/B` | 接通后时间线执行中工具行转发 CancelToolCall |
| C | `vscode-WorkTrees/C` | `loop/C` | AgentService.CancelToolCall catalog + node unary；时间线转发见 B |
