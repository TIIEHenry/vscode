...
phase: M7
updated: 2026-09-14
summary: "工位 D `dadcaf3107e`；本树 D445 Navigator leftover Inspect/Reveal/row-open/session-switch chrome 已写未合；D405 仍开；A 的 D444 Sources FileDiff leftover 不在本树；下号 D446"
---
...
### 进行中（2026-09-14 · 工位 D 写 D445，未合）
| 槽 | 切片 | 状态 |
|:---|:-----|:---------|
| **A** | D444 Sources FileDiff leftover（A 树未合） | 不在本树；勿改 Sources |
| **B** | — | 脏 `worktree-pool.md` 勿 add / 勿 `-B` |
| **C** | — | 脏 `dev/loop`；勿 add / 勿 `-B` |
| **D** | D445 Navigator leftover Inspect / Reveal / row-open / leftover session switch chrome | 已写未合；Agents + Team + Projects；不宣称 leftover wave 完成 |
...
| [D405](deferred-gaps.md)–[D445](deferred-gaps.md) | A/B/D 本波 | D405 手测仍开；D413–D443 closed；本树 **D445** Navigator leftover Inspect/Reveal/开行/切会话已写未合；A 的 D444 不在本树；不得宣称 leftover wave 完成 |
## 工位表（P7 · 2026-09-14 · 与 `git worktree list` 对照）
| 槽 | 路径 | 分支 | tip | 脏 | stash | 关仓状态 |
|----|------|------|-----|:--|:------|:---------|
| merge | `vscode-WorkTrees/merge` | `loop/merge` | `4307a39542e` | 干净 | 0 | `parked`；MERGE_SHA；compile-client 0；已 push |
| A | `vscode-WorkTrees/A` | `loop/A` | `b9c558e0087` | 干净 | 0 | `merge-queued` D444；勿改 Sources |
...
| D | `vscode-WorkTrees/D` | `loop/D` | `dadcaf3107e` | 脏 D445 | 0 | `merge-queued` 待本地 commit |
...
| **loop 切片** | 工位 D @ `dadcaf3107e`。本树 **D445** 已写未合（Navigator leftover action chrome）。A 的 **D444** Sources FileDiff leftover 不在本树。下号 **D446**。**D405** 手测仍开。不关 D8/D16/D147。未升 PRD-008。不得宣称 leftover wave 完成。 |
