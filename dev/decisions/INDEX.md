...
phase: N/A
updated: 2026-09-07
summary: "本仓库本地 ADR 索引；ADR-008 WriteGitApplyHunks 空列表语义 review（R8 仍开）；ADR-007 上游同步 accepted；ADR-005 Diff owner 已接受；ADR-004 已被 ADR-005 取代；ADR-003 引擎 adapter 边界 accepted"
---
...
| [003-engine-adapter-boundary.md](003-engine-adapter-boundary.md) | `accepted` | UA 传输落 platform/universeAgent；roster 同 token `'conversationStubService'`；AHP 非权威；stream-timeline §9 已并入；宿主 = electron-main @2026-09-02 |
...
| [008-write-git-apply-hunks-empty.md](008-write-git-apply-hunks-empty.md) | `review` | R8：本仓无 `git_service.proto`；空 `argv`/`patches` 只保证原样上线，引擎语义（apply-all / 拒写 / 成功 no-op）未定；Sources Accept 不发明 hunk；D31 F4 未跑 |
