...
**默认窗口 Activity 门闩（2026-08-31）**：Run and Debug（`workbench.view.debug` / `VIEWLET_ID`）Sidebar 容器与 Variables / Watch / Call Stack / Breakpoints / Welcome 视图在默认 Code 窗口以 `IsSessionsWindowContext` 门闩隐藏（`hideIfEmpty: true`；`Ctrl+Shift+D` 与 View 菜单 open command 同步门闩）；Debug Console（`workbench.panel.repl`）仍留在 Panel。Agents Window 保留 Run and Debug Sidebar 与快捷键。Extensions（`workbench.view.extensions`）Sidebar 容器与 Installed / Popular / Recommended / Marketplace 等视图同样在默认 Code 窗口以 `IsSessionsWindowContext` 门闩隐藏（`hideIfEmpty: true`；`Ctrl+Shift+X` 与 View 菜单 open command 同步门闩）；Extension Editor / Command Palette「Extensions: Install」等仍可用。Agents Window 保留 Extensions Sidebar 与快捷键。
