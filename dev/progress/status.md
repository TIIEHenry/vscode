...
phase: M7
updated: 2026-09-07
summary: "工位 B 占 create-scoped。S1a scoped 测已绿（harness 补 IStatusbarService.createScoped）；名单 31 行。D16 仍开。A2 仍 blocked。"
---
...
1. **集成 tip** 以 merge 本关仓提交为准（切片 3 A=`c01e16fad84` → merge `44d65d1da44`；unit-custom XML A=`a167f755fb4` → merge `7939b930798`；DiffReview B=`ebe958ab148` → merge `10722d6695a`）。`npm run compile` 仍基线 unused 红则 **不 push**。GFS >800 不拆。D22/F3 已撤回。不跑 F4 / 不实施 A2。
2. 本波字母槽已进 merge：
   - **A**：`unit-custom-xml` — [`scripts/run-unit-custom.sh`](../../scripts/run-unit-custom.sh) 排除 D17 十一份 Electron 不可加载 node 测，一次 `--tfs universeAgent` 写出 XML；conversation / sources 仍官方 glob。切片 3 名单已落。**[D16](deferred-gaps.md) 仍开**；未开切片 1。未改 Electron runner / `min_cases`。
   - **B**：`create-scoped` — S1a `scoped instantiation services are unique per conversation part` 已绿。分类 **A**（夹具）：`createConversationEditorPart` 已按 part 调 `editorService.createScoped` 并写入 map；测败在 `getScopedInstantiationService(mainPart)` 懒建时 `IStatusbarService` 未 stub。`createHarness` 补 `createScoped` stub，**未缩** uniqueness 断言。名单删该一行（31 行）。**[D16](deferred-gaps.md) 仍开**。未开切片 1 / 未改 Electron runner / `min_cases`。
...
| B | `vscode-WorkTrees/B` | `loop/B` | 对齐本关仓提交 | `create-scoped` 未提交 | 0 | occupy `create-scoped` |
...
| **test-baseline** | 名单 31 行（B `create-scoped` 删 S1a scoped 一行）；unit-custom 排除 11 unloadable。**D16 仍开**；勿开切片 1「已归零」 |
