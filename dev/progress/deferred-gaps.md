...
created: 2026-08-30
updated: 2026-09-07
summary: "延期缺口 SSOT；D16 仍开（切片 3 名单已落，三文件标题未列入；compile/S2 leftover 不闭）；D17 leftover 夹具已修；D42 最大化只藏对话树；D43 ConversationPart 尺寸扇出已收；D22 F3；D24 其余 JSON RPC；D25 引擎 List 真空；D26 引擎建壳回 6；D31 F4 / A2 blocked"
---
...
| D42 | P3 | **Maximize 后切 Trajectory 虚列表仍不铺行**。CSS 合同已改：只藏 `.conversation-lens-timeline`，轨迹页不整槽 `display:none`（merge 源码合同测绿）。活测 `input maximize hides conversation tree but still paints Trajectory rows` 在 merge compile 后仍 `expected trajectory row untitled-u1` | CSS 选择器已收；铺行未证 | 最大化后切 Trajectory 仍能见 `untitled-u1` 行（merge compile 复证） | conversation | open |
