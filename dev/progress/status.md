...
phase: M7
updated: 2026-09-08
summary: "D16 / D31 F4 仍开。官方三域 glob 已绿。D44–D90 / D92–D122 已闭。bind acquire throw 已 failed；Enqueue 断连+history 可点出 engine_disconnected。A2 仍 blocked。"
---
...
62. **D 槽 `lens-bind-acquire-throw-catch`（未关 D16）**：`bindSessionView` 包 `acquireSessionView` try/catch；throw 画 `showPostFailure('failed')`、lease 置空，不打断 `applyActiveSession`。[D122](deferred-gaps.md) **已闭**。未 compile。
子 agent 发现的既有代码问题：
| ID | 来源 | 问题 |
|:---|:-----|:-----|
...
| [D122](deferred-gaps.md) | D 槽 `lens-bind-acquire-throw-catch` | **closed** bind acquire throw 已 `showPostFailure('failed')`；不留半应用 lease；未关 D16 |
