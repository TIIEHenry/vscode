...
| `SOURCES_PART` | `workbench.parts.sources` | **End 列下格**：`contrib/sources` **Files \| Changes \| Review** tab strip（Files = 只读列表投影；Changes = SCM 变更资源列表 → Preview；Review = SCM 变更资源只读列表 → Preview（`SourcesReviewList`）；树权威仍在 Sidebar Explorer） |
...
**锚点事实：** 中间枝的「主面积」叶子是 `{ type: Parts.CONVERSATION_PART }`。Editor 仍走 `IEditorService.openEditor`，出现在 End 列上格原生 tabs；Sources 占 End 下格，`contrib/sources` 提供 **Files \| Changes \| Review** tab strip（Files 为只读列表投影；Changes = SCM 变更资源列表 → Preview；Review = SCM 变更资源只读列表 → Preview）。
...
7. **Sources tabs** — `contrib/sources`：title 区 **Files \| Changes \| Review** tab strip；Files = `SourcesFilesList` 只读列表投影（点击 `IEditorService.openEditor` 落 End 上格 Preview）；Changes = `SourcesChangesList` SCM 变更资源列表（同样 `openEditor` 打开文件，**不**走 Diff / `openDiff`）；Review = `SourcesReviewList` SCM 变更资源只读列表（同样 `openEditor` 打开 Preview，**不**走 Diff）；Diff 深查看仍 **EDITOR_PART** FORK。
