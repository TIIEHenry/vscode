...
| `SessionService` | `List` / `Create` / `Delete` / `Info` / `Resume` / `Shelve` / `Unshelve` / `Purge` / `ResolveAnchor` / `GetHistory` / `SessionEventStream` | Conversation roster + 时间线 fold 输入（`GetHistory` + 流 → session-core Actor → `ViewFrame`）。`Info` / `Resume` / `Shelve` / `Unshelve` / `Purge` / `ResolveAnchor` 仅 catalog + node | 无 `SwitchSession`；切换 = IDE 客户端投影。标题 proto 为 `AgentService.Rename`（catalog + node unary + **roster 接通后转 unary**）。`subscribeSessionEventStream` 第三参 `onClosed` 与 Chat bidi 同因（`remote` / `error`）；本地 dispose 不回调。宿主 `openStream` 把 remote/error 经 `postAndDrain` 折成 Actor `streamClosed` |
