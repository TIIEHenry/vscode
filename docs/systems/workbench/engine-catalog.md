...
phase: N/A
updated: 2026-09-02
summary: "M6-C E1 @ HEAD（`f49615a1`）：Skills list/toggle；Agents/MCP/Tools List + MCP toggle；Agents Save/Delete/Reset、MCP Add/Update/Remove、Tools 经 SaveAgentProfile 写路径已进传输与 Engine 页写入口；Skill 正文/新建、MCP 运行态、Plugins 仍待"
---
...
Engine Preferences 子页（`EnginePreferencesPane`）承载 Customizations 产品主面。本节记 **@ HEAD（`f49615a1`）已落地的 list/toggle 与写路径消费面**；不得把未合入的 Skill 正文编辑器、MCP 运行态探针或 Plugins 节写成已接通。
...
| **Agents** | `ListAgentProfiles` · `SaveAgentProfile` · `DeleteAgentProfile` · `ResetAgentProfile` | `EngineAgentsSection`：project/user/built_in 分组列表；`supported` 时写工具栏 **New / Delete / Reset**（Delete 禁 built_in；Reset 仅 built_in） | list @ `4833c008`；写 RPC + 写入口 @ `7f10e65c` / `f49615a1`；**无** `AGENTS.md` 全文编辑器 |
...
单测：`engineCatalogSections.test.ts` · `engineSkillsSection.test.ts` · `engineToolProfile.test.ts`（能力三态、断连路径与写 RPC 形状）。

## 4. 未落地（本文不得写「已接」）

| 操作 | RPC / 面 | 状态 |
|------|----------|------|
| Agent profile 正文编辑 | `SaveAgentProfile` → `AGENTS.md` 全文 | 写 RPC 已进传输；Engine 页 **无** 目录三件套编辑器（仅程序化 New 与 Delete/Reset 工具栏） |
