...
phase: M7
updated: 2026-09-08
summary: "D104 已闭（saveTurnEdit false 先 notice、不退编辑）。D16 / D31 F4 仍开。官方三域 glob 已绿。D44–D90 / D92–D102 已闭。A2 仍 blocked。"
---
...
62. **B 槽 `composer-turn-edit-save-rejected-notice`（未关 D16 / 未改名单 / 未改 `submitDraft` / 未改 roster `editEngineMessage`）**：`saveTurnEdit` 先看 `updateUserTurnText`；false 画 `engine_disconnected`/`failed`、不 `exitComposerEdit`；true 仍退出。[D104](deferred-gaps.md) **已闭**。
子 agent 发现的既有代码问题：
| ID | 来源 | 问题 |
|:---|:-----|:-----|
...
| [D104](deferred-gaps.md) | B 槽 `composer-turn-edit-save-rejected-notice` | **closed** saveTurnEdit false 先 notice、不退编辑；true 仍退出；未关 D16 |
