...
phase: M7
updated: 2026-09-07
summary: "A 占 unit-custom-xml。三域基线名单已落（38 行）。D16 仍开。A2 仍 blocked"
---
...
2. 本波字母槽已进 merge：
   - **A**：`unit-custom-xml` — [test-baseline-ci](../plans/test-baseline-ci.md) unit-custom：[`scripts/run-unit-custom.sh`](../../scripts/run-unit-custom.sh) 排除 D17 十一份 Electron 不可加载 node 测，一次 `--tfs universeAgent` 写出 XML；conversation / sources 仍官方 glob。**[D16](deferred-gaps.md) 仍开**；未开切片 1「已归零」。未改 Electron runner / 未改 `min_cases` / 未跑 compile。
...
| A | `vscode-WorkTrees/A` | `loop/A` | `d98e699a11f` | 本 slice | 0 | occupy `unit-custom-xml` |
...
| **test-baseline 切片 3 / unit-custom XML** | [test-baseline-failures.txt](test-baseline-failures.txt) 已落 @ `d2abb648c0e`。A 占 `unit-custom-xml`（排除 11 unloadable）。**D16 仍开**；勿开切片 1「已归零」 |
