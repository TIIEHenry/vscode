...
| R9 | 引擎正文会话链接格式 | [conversation-timeline-session-pills](../plans/conversation-timeline-session-pills.md) §3.1 规定引擎字面链接须由帧源改写成 `conversation-chat:`；本仓不知引擎给什么 scheme / path，不能发明 RPC | 对照 UA 帧源 / proto 时间线文本：会话或子代理引用的字面格式、是否已带稳定 id、能否在投影前解析 `chatId` | 登记格式 + 改写规则写入帧源方案（闭合 [D463](deferred-gaps.md)）；不升 PRD-008 | open |
