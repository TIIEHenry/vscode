...
phase: M7
updated: 2026-09-16
summary: "D 槽 Agent.History wire 已落，getAgentHistory 仍 JSON。D24 仍开。不是 leftover/pills 完成。"
---
...
| 槽 | 状态 |
|:---|:-----|
| **D** | `d24-agent-history-unary-wire`：Agent.History wire+测已落；`getAgentHistory` 仍 JSON；未接线；D24 仍开 |
...
| ID | 问题 |
|:---|:-----|
| [D24](deferred-gaps.md) | **仍开**：Agent.History wire 已落，`getAgentHistory` 仍 JSON；SwitchWorkDir/Compact/Todo/Usage/Status wire 未接线；ResolveTurn/Connect/SaveSkillContent/Watch·Rebuild 仍 JSON |
...
| 项 | 指针 |
|:---|:-----|
| **loop** | D24 仍开。Agent.History wire 已落、`getAgentHistory` 仍 JSON。A 下一刀接 SwitchWorkDir/Compact/Todo/Usage/Status 或 History 接线。SaveSkillContent 无 RPC。Connect/Watch 跳过。不得宣称 leftover/pills 完成 |
