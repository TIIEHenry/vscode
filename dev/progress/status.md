...
- **槽 C / `loop/C`：** 把 `SessionService.ResolveAnchor` 写入 gRPC catalog（`universeagent.session.v1.SessionService`），node unary `resolveAnchor`（snake_case `anchor.session_id`/`anchor.envelope_id`/`anchor.generation?`/`scope`/`current_leaf_turn_id?`；响应 oneof `hit`/`tombstone`/`expired`）。合同可选。空 `sessionId` / `envelopeId` / `currentLeafTurnId` 原样上线。**不改** Conversation roster / SessionBar / Snapshots overlay。≠ `ResolveTurn` / `GetHistory` / `Compact` / `Export` / `Purge` / `Todo`。测：catalog + 转发 / 空 id。
- **槽 D / `loop/D`：** 把 `AgentService.Todo` 写入 gRPC catalog（`universeagent.agent.v1.AgentService`），node unary `getTodo`（snake_case `session_id`/`agent_id`；响应 `items[]`：`id`/`content`/`status`/`priority`/`require_confirm`/`blocked`）。合同可选。空 `sessionId` / `agentId` 原样上线。**不改** Conversation roster / SessionBar / Snapshots overlay。≠ `Status`（B 槽）/ `Tree` / `Compact`。测：catalog + 转发 / 空 id。
...
| C | `vscode-WorkTrees/C` | `loop/C` | SessionService.ResolveAnchor catalog + node unary resolveAnchor；空 sessionId / envelopeId / currentLeafTurnId 原样上线；无 Conversation UI |
| D | `vscode-WorkTrees/D` | `loop/D` | AgentService.Todo catalog + node unary getTodo；空 sessionId / agentId 原样上线；无 Conversation UI |
