...
class MockUniverseAgentGrpcTransport implements IUniverseAgentGrpcTransport {
	...
	readonly listNodesCalls: UniverseAgentListNodesRequest[] = [];
	listNodesResult: UniverseAgentListNodesResult = {
		nodes: [],
		total: 0,
		onlineCount: 0,
	};
	...
	async listNodes(request: UniverseAgentListNodesRequest): Promise<UniverseAgentListNodesResult> {
		this.listNodesCalls.push(request);
	...
	readonly setMaintenanceCalls: UniverseAgentSetMaintenanceRequest[] = [];
	setMaintenanceResult: UniverseAgentSetMaintenanceResult = {
		success: false,
	};

	async setMaintenance(request: UniverseAgentSetMaintenanceRequest): Promise<UniverseAgentSetMaintenanceResult> {
		this.setMaintenanceCalls.push(request);
		return this.setMaintenanceResult;
	}

	readonly exitMaintenanceCalls: UniverseAgentExitMaintenanceRequest[] = [];
	exitMaintenanceResult: UniverseAgentExitMaintenanceResult = {
		success: false,
	};

	async exitMaintenance(request: UniverseAgentExitMaintenanceRequest): Promise<UniverseAgentExitMaintenanceResult> {
		this.exitMaintenanceCalls.push(request);
		return this.exitMaintenanceResult;
	}

	listConfigsCalls = 0;
	listConfigsResult: UniverseAgentListConfigsResult = {
		configs: [],
	};

	async listConfigs(): Promise<UniverseAgentListConfigsResult> {
		this.listConfigsCalls++;
	...
	readonly destroyRemoteSessionCalls: UniverseAgentDestroyRemoteSessionRequest[] = [];
	destroyRemoteSessionResult: UniverseAgentDestroyRemoteSessionResult = {
		success: false,
		message: '',
	};
	...
	async destroyRemoteSession(request: UniverseAgentDestroyRemoteSessionRequest): Promise<UniverseAgentDestroyRemoteSessionResult> {
		this.destroyRemoteSessionCalls.push(request);
	...
	readonly getRemoteSessionHistoryCalls: UniverseAgentGetRemoteSessionHistoryRequest[] = [];
	getRemoteSessionHistoryResult: UniverseAgentGetRemoteSessionHistoryResult = {
		messages: [],
		version: 0,
		hasMore: false,
	};
	...
	async getRemoteSessionHistory(request: UniverseAgentGetRemoteSessionHistoryRequest): Promise<UniverseAgentGetRemoteSessionHistoryResult> {
		this.getRemoteSessionHistoryCalls.push(request);
		return this.getRemoteSessionHistoryResult;
	}
	readonly resumeRemoteSessionCalls: UniverseAgentResumeRemoteSessionRequest[] = [];
	resumeRemoteSessionResult: UniverseAgentResumeRemoteSessionResult = {
		success: false,
		callId: '',
		status: '',
		message: '',
	...
	async resumeRemoteSession(request: UniverseAgentResumeRemoteSessionRequest): Promise<UniverseAgentResumeRemoteSessionResult> {
		this.resumeRemoteSessionCalls.push(request);
		return this.resumeRemoteSessionResult;
	}
	readonly cancelRemoteSessionCalls: UniverseAgentCancelRemoteSessionRequest[] = [];
	cancelRemoteSessionResult: UniverseAgentCancelRemoteSessionResult = {
		success: false,
		callId: '',
		status: '',
		message: '',
	...
	async cancelRemoteSession(request: UniverseAgentCancelRemoteSessionRequest): Promise<UniverseAgentCancelRemoteSessionResult> {
		this.cancelRemoteSessionCalls.push(request);
		return this.cancelRemoteSessionResult;
	}
...
suite('UniverseAgentConnectionService', () => {
	...
	test('listNodes forwards request and maps result', async () => {
		const transport = new MockUniverseAgentGrpcTransport();
		const service = new UniverseAgentConnectionService({
			createTransport: () => transport,
		});
		...
		const result = await service.listNodes(request);
		assert.deepStrictEqual(transport.listNodesCalls, [request]);
		assert.deepStrictEqual(result, transport.listNodesResult);
		...
		const empty = await service.listNodes(emptyRequest);
		assert.deepStrictEqual(transport.listNodesCalls[1]?.filterStatus, ['']);
		assert.deepStrictEqual(transport.listNodesCalls[1]?.filterTags, ['']);
		assert.strictEqual(empty.nodes[0]?.id, '');
		assert.strictEqual(empty.nodes[0]?.name, '');
		assert.strictEqual(empty.nodes[0]?.description, '');
		assert.strictEqual(empty.nodes[0]?.status, '');
		assert.strictEqual(empty.nodes[0]?.endpoint, '');
		assert.deepStrictEqual(empty.nodes[0]?.tags, ['']);
		assert.strictEqual(empty.nodes[0]?.capabilities.models[0]?.id, '');
		assert.strictEqual(empty.nodes[0]?.capabilities.models[0]?.enabled, false);
		assert.strictEqual(empty.nodes[0]?.capabilities.serverVersion, '');
		assert.strictEqual(empty.nodes[0]?.capabilities.properties[''], '');
		assert.strictEqual(empty.nodes[0]?.load.activeSessions, 0);
		assert.strictEqual(empty.nodes[0]?.lastHeartbeatAt, 0);
		assert.strictEqual(empty.total, 0);
		assert.strictEqual(empty.onlineCount, 0);
	...
	test('setMaintenance forwards request and maps result', async () => {
		const transport = new MockUniverseAgentGrpcTransport();
		const service = new UniverseAgentConnectionService({
			createTransport: () => transport,
		});
		...
		transport.setMaintenanceResult = {
			success: true,
		};
		const request = {
			nodeId: 'node-1',
		};
		const result = await service.setMaintenance(request);
		assert.deepStrictEqual(transport.setMaintenanceCalls, [request]);
		assert.deepStrictEqual(result, transport.setMaintenanceResult);
		...
		const empty = await service.setMaintenance(emptyRequest);
		assert.strictEqual(transport.setMaintenanceCalls[1]?.nodeId, '');
		assert.strictEqual(empty.success, false);
		service.dispose();
	});

	test('exitMaintenance forwards request and maps result', async () => {
		const transport = new MockUniverseAgentGrpcTransport();
		const service = new UniverseAgentConnectionService({
			createTransport: () => transport,
		});
		...
		transport.exitMaintenanceResult = {
			success: true,
		};
		const request = {
			nodeId: 'node-1',
		};
		const result = await service.exitMaintenance(request);
		assert.deepStrictEqual(transport.exitMaintenanceCalls, [request]);
		assert.deepStrictEqual(result, transport.exitMaintenanceResult);
		...
		const empty = await service.exitMaintenance(emptyRequest);
		assert.strictEqual(transport.exitMaintenanceCalls[1]?.nodeId, '');
		assert.strictEqual(empty.success, false);
		service.dispose();
	});

	test('listConfigs forwards empty request and maps result', async () => {
		const transport = new MockUniverseAgentGrpcTransport();
		const service = new UniverseAgentConnectionService({
			createTransport: () => transport,
		});
		...
		assert.strictEqual(transport.listConfigsCalls, 1);
		assert.deepStrictEqual(result, transport.listConfigsResult);
		...
		const empty = await service.listConfigs();
		assert.strictEqual(transport.listConfigsCalls, 2);
		assert.strictEqual(empty.configs[0]?.id, '');
		assert.strictEqual(empty.configs[0]?.name, '');
		assert.strictEqual(empty.configs[0]?.description, '');
		assert.strictEqual(empty.configs[0]?.enabled, false);
		assert.strictEqual(empty.configs[0]?.endpoint.host, '');
		assert.strictEqual(empty.configs[0]?.endpoint.port, 0);
		assert.strictEqual(empty.configs[0]?.endpoint.tls, false);
		assert.strictEqual(empty.configs[0]?.endpoint.tlsCertPath, '');
		assert.strictEqual(empty.configs[0]?.auth.type, '');
		assert.strictEqual(empty.configs[0]?.auth.apiKeyRef, '');
		assert.strictEqual(empty.configs[0]?.auth.tokenRef, '');
		assert.deepStrictEqual(empty.configs[0]?.tags, ['']);
		assert.strictEqual(empty.configs[0]?.maxConcurrentSessions, 0);
		assert.strictEqual(empty.configs[0]?.sessionLifecycle, '');
		assert.strictEqual(empty.configs[0]?.defaultPermissionDelegate.mode, '');
		assert.strictEqual(empty.configs[0]?.defaultPermissionDelegate.whitelist[0]?.toolName, '');
		assert.strictEqual(empty.configs[0]?.defaultPermissionDelegate.whitelist[0]?.argConditions[0]?.field, '');
		assert.strictEqual(empty.configs[0]?.defaultPermissionDelegate.budget.maxToolCalls, 0);
		assert.strictEqual(empty.configs[0]?.healthCheck.intervalMs, 0);
		assert.strictEqual(empty.configs[0]?.healthCheck.useWatch, false);
		assert.strictEqual(empty.configs[0]?.healthCheck.degradedErrorRateThreshold, 0);
	...
	test('destroyRemoteSession forwards request and maps result', async () => {
		const transport = new MockUniverseAgentGrpcTransport();
		const service = new UniverseAgentConnectionService({
			createTransport: () => transport,
		});
		...
		transport.destroyRemoteSessionResult = {
			success: true,
			message: 'destroyed',
		};
		const request = {
		...
		const result = await service.destroyRemoteSession(request);
		assert.deepStrictEqual(transport.destroyRemoteSessionCalls, [request]);
		assert.deepStrictEqual(result, transport.destroyRemoteSessionResult);
		...
		const empty = await service.destroyRemoteSession(emptyRequest);
	...
	test('getRemoteSessionHistory forwards request and maps result', async () => {
		const transport = new MockUniverseAgentGrpcTransport();
		const service = new UniverseAgentConnectionService({
			createTransport: () => transport,
		});
		...
		transport.getRemoteSessionHistoryResult = {
			messages: [
				{ system: { content: 'sys' } },
				{ user: { content: 'hi' } },
				{ assistant: { content: 'ok', toolCalls: [{ id: 'tc-1', name: 'read', arguments: '{}' }] } },
				{ toolResult: { toolCallId: 'tc-1', toolName: 'read', content: 'done', isError: false } },
			],
			version: 7,
			hasMore: true,
		};
		...
		const result = await service.getRemoteSessionHistory(request);
		assert.deepStrictEqual(transport.getRemoteSessionHistoryCalls, [request]);
		assert.deepStrictEqual(result, transport.getRemoteSessionHistoryResult);

		transport.getRemoteSessionHistoryResult = {
			messages: [
				{ system: { content: '' } },
				{ user: { content: '' } },
				{ assistant: { content: '', toolCalls: [{ id: '', name: '', arguments: '' }] } },
				{ toolResult: { toolCallId: '', toolName: '', content: '', isError: false } },
			],
			version: 0,
			hasMore: false,
		};
		...
		const empty = await service.getRemoteSessionHistory(emptyRequest);
		assert.strictEqual(transport.getRemoteSessionHistoryCalls[1]?.callId, '');
		assert.strictEqual(transport.getRemoteSessionHistoryCalls[1]?.sinceVersion, 0);
		assert.strictEqual(transport.getRemoteSessionHistoryCalls[1]?.pageSize, 0);
		assert.strictEqual(empty.messages[0]?.system?.content, '');
		assert.strictEqual(empty.messages[1]?.user?.content, '');
		assert.strictEqual(empty.messages[2]?.assistant?.content, '');
		assert.strictEqual(empty.messages[2]?.assistant?.toolCalls[0]?.id, '');
		assert.strictEqual(empty.messages[2]?.assistant?.toolCalls[0]?.name, '');
		assert.strictEqual(empty.messages[2]?.assistant?.toolCalls[0]?.arguments, '');
		assert.strictEqual(empty.messages[3]?.toolResult?.toolCallId, '');
		assert.strictEqual(empty.messages[3]?.toolResult?.toolName, '');
		assert.strictEqual(empty.messages[3]?.toolResult?.content, '');
		assert.strictEqual(empty.messages[3]?.toolResult?.isError, false);
		...
		const emptyList = await service.getRemoteSessionHistory(emptyRequest);
		assert.strictEqual(transport.getRemoteSessionHistoryCalls[2]?.callId, '');
		assert.deepStrictEqual(emptyList.messages, []);
		service.dispose();
	});
	test('resumeRemoteSession forwards request and maps result', async () => {
		const transport = new MockUniverseAgentGrpcTransport();
		const service = new UniverseAgentConnectionService({
			createTransport: () => transport,
		});
		...
		transport.resumeRemoteSessionResult = {
			success: true,
			callId: 'call-1',
			status: 'resumed',
			message: 'ok',
		...
		const result = await service.resumeRemoteSession(request);
		assert.deepStrictEqual(transport.resumeRemoteSessionCalls, [request]);
		assert.deepStrictEqual(result, transport.resumeRemoteSessionResult);
		...
		const empty = await service.resumeRemoteSession(emptyRequest);
		assert.strictEqual(transport.resumeRemoteSessionCalls[1]?.callId, '');
		assert.strictEqual(transport.resumeRemoteSessionCalls[1]?.nodeId, '');
		assert.strictEqual(empty.success, false);
		assert.strictEqual(empty.callId, '');
		assert.strictEqual(empty.status, '');
		assert.strictEqual(empty.message, '');
	...
	test('cancelRemoteSession forwards request and maps result', async () => {
		const transport = new MockUniverseAgentGrpcTransport();
		const service = new UniverseAgentConnectionService({
			createTransport: () => transport,
		});
		...
		transport.cancelRemoteSessionResult = {
			success: true,
			callId: 'call-1',
			status: 'cancelled',
			message: 'cancelled',
		...
		const result = await service.cancelRemoteSession(request);
		assert.deepStrictEqual(transport.cancelRemoteSessionCalls, [request]);
		assert.deepStrictEqual(result, transport.cancelRemoteSessionResult);
		...
		const empty = await service.cancelRemoteSession(emptyRequest);
		assert.strictEqual(transport.cancelRemoteSessionCalls[1]?.callId, '');
		assert.strictEqual(transport.cancelRemoteSessionCalls[1]?.reason, '');
		assert.strictEqual(empty.success, false);
		assert.strictEqual(empty.callId, '');
		assert.strictEqual(empty.status, '');
		assert.strictEqual(empty.message, '');
	...
	test('openRemoteChatStream forwards request and transport onClosed', async () => {
		...
		assert.deepStrictEqual(transport.remoteChatOpens, [{
			...
			overridePending: true,
		}] as UniverseAgentRemoteChatRequest[]);
...
suite('UniverseAgentConnectionService reconnect backoff (D408)', () => {
	...
	test('user reconnect resolve() throw reschedules (D417)', async () => {
		const clock = createReconnectClock();
		let resolves = 0;
		const resolver = {
			resolve: async () => {
				...
				throw new Error('resolve blowup');
			},
			createIssueRelayTicketHook: () => async () => ({ ok: false as const, code: 'hub_session_required' as const, reason: 'test' }),
		};
		const service = new UniverseAgentConnectionService({
		...
		await assert.rejects(
			() => service.connectProfile('p1', { reconnect: true }),
			(error: unknown) => error instanceof Error && error.message === 'resolve blowup',
		);
		assert.deepStrictEqual(clock.delays, [1000, 2000]);
		assert.strictEqual(clock.pending.size, 1);
		service.dispose();
	...
	test('user Connect after failed transport getOrCreateIdentity throw reschedules (D417)', async () => {
		const clock = createReconnectClock();
		let identities = 0;
		const throwingIdentity: IClientIdentityStore = {
			...identityStore,
			getOrCreateIdentity: async () => {
				identities++;
				if (identities === 1) {
				...
				throw new Error('identity blowup');
			},
		};
		const service = new UniverseAgentConnectionService({
			createTransport: () => new MockUniverseAgentGrpcTransport({
			...
			connectionResolver: createOkResolver() as unknown as ConnectionResolver,
			clientIdentityStore: throwingIdentity,
			reconnectJitterRatio: 0,
			setTimeoutFn: clock.setTimeoutFn,
			clearTimeoutFn: clock.clearTimeoutFn,
		});
		...
		await assert.rejects(
			() => service.connectProfile('p1'),
			(error: unknown) => error instanceof Error && error.message === 'identity blowup',
		);
		assert.deepStrictEqual(clock.delays, [1000, 2000]);
		assert.strictEqual(clock.pending.size, 1);
		service.dispose();
	...
	test('user reconnect createSigner throw reschedules (D417)', async () => {
		const clock = createReconnectClock();
		let resolves = 0;
		const resolver = {
			resolve: async () => {
		...
		const throwingSigner: IClientIdentityStore = {
			...signingIdentityStore,
			createSigner: async () => {
				throw new Error('signer blowup');
			},
		};
		const service = new UniverseAgentConnectionService({
			createTransport: () => new MockUniverseAgentGrpcTransport({
			...
			createPinnedTransport: () => new MockUniverseAgentGrpcTransport(),
			connectionResolver: resolver as unknown as ConnectionResolver,
			connectionProfileStore: new PairingTestProfileStore(handshakeProfile),
			clientIdentityStore: throwingSigner,
			reconnectJitterRatio: 0,
			setTimeoutFn: clock.setTimeoutFn,
			clearTimeoutFn: clock.clearTimeoutFn,
		});
		...
		await assert.rejects(
			() => service.connectProfile('p1', { reconnect: true }),
			(error: unknown) => error instanceof Error && error.message === 'signer blowup',
		);
		assert.deepStrictEqual(clock.delays, [1000, 2000]);
		assert.strictEqual(clock.pending.size, 1);
		service.dispose();
	...
	test('initial resolve() throw does not schedule (D417)', async () => {
		const clock = createReconnectClock();
		const resolver = {
			resolve: async () => {
				throw new Error('resolve blowup');
	...
	test('user reconnect resolve() throw while pairingPending does not schedule (D417)', async () => {
		const clock = createReconnectClock();
		let resolves = 0;
		const resolver = {
			resolve: async () => {
