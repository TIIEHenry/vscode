...
phase: M7
updated: 2026-09-16
summary: "G File unary wire+测（未接 grpcClient）。D24 仍开（Memory / ChatSync JSON）。不是 leftover/pills 完成。R9/D405 仍开"
---
...
| **A/D** | D24 question/client-tool + D481 | 已合；D24 **仍开**（Memory / ChatSync JSON）。D481 **closed** |
| **B/C/E** | — | B 脏 `worktree-pool.md`；C 脏 `dev/loop` gitlink；E leftover blocked 勿 `-B` |
<details>
...
| [D24](deferred-gaps.md) | G 槽 File unary wire（未接 grpcClient） | **仍开**：File 六条 wire+测已写；grpcClient 仍 JSON；Memory / ChatSync 仍 JSON |
...
| [D405](deferred-gaps.md)–[D481](deferred-gaps.md) | A/D 本波 | D405 手测仍开；D471–D481 已闭；**D24 仍开**（File wire 未接 grpcClient；Memory / ChatSync 仍 JSON）；不得宣称 leftover/pills 完成 |
...
| G | `vscode-WorkTrees/G` | `loop/G` | `731cf18` | File wire | 0 | File unary wire+测；未接 grpcClient；D24 仍开 |
| edit | `Projects/Agents/vscode` | `agent-ide` | `ff278772b85` | `dev/loop`+docs | 0 | 仅 ff-only |
## Next（Blockers：无）
| 项 | 指针 |
|:---|:-----|
...
| **loop 切片** | G File 六条 wire+测（未接 grpcClient）。[D24](deferred-gaps.md) **仍开**（Memory / ChatSync JSON）。D481 **closed**。R9 / D405 仍开。不关 D8/D16/D147。未升 PRD-008。不得宣称 leftover / pills 完成。 |
