...
phase: M7
updated: 2026-09-08
summary: "D16 / D31 F4 仍开。官方三域 glob 已绿。D44–D90 / D92–D122 / D124 已闭。Create Snapshot false 已 notice；live-tree / bind / timeline reveal throw 已吞。A2 仍 blocked。"
---
...
61. **D100–D122**（未关 D16）：peek / close / opener / stale / save·delete·cancel·Stop·Goal·Enqueue false notice / reveal / open-beside / primary bootstrap notice；Goal / Enqueue 断连+history 可点；permission/question roster false；SessionBar rename/delete 与 Sessions 侧栏 delete false notice；notifications / live-tree / bind acquire throw → notice；Create Snapshot false notice。
62. **B 槽 `timeline-reveal-service-throw-catch`（未关 D16）**：reveal 服务 lens 委托 try/catch → notice；scroll 仍 coalescing。[D124](deferred-gaps.md) **已闭**。未改 revealItem contribution / openPending / Inbox / navigator lease。
子 agent 发现的既有代码问题：
| ID | 来源 | 问题 |
|:---|:-----|:-----|
...
| [D92](deferred-gaps.md)–[D122](deferred-gaps.md) | B roster+inbox+sessions / D nav / A beside+mru+sessionbar / A·B composer / A notifications / B sessionChat / D bind | **closed** throw 回滚/notice；peek；closeEditors；opener 回滚；stale；save/delete/cancel/Stop/Goal/Enqueue false 先 notice；reveal acquire；open-beside / primary bootstrap catch error notice；Goal / Enqueue 断连+history 可点；permission/question roster false；SessionBar rename/delete 与 Sessions 侧栏 delete false notice；notifications / live-tree / bind acquire throw → notice；Create Snapshot false notice；未关 D16 |
| [D124](deferred-gaps.md) | B 槽 `timeline-reveal-service-throw-catch` | **closed** lens throw → error notice；无未处理 rejection；二次仍跑；未关 D16 |

## 工位表（P0 盘点 · 2026-09-08 · 与 `git worktree list` 对照）
| 槽 | 路径 | 分支 | tip | 脏 | stash | 关仓状态 |
|----|------|------|-----|:--|:------|:---------|
| merge | `vscode-WorkTrees/merge` | `loop/merge` | `bb53e1778aa` | `__pycache__` | 0 | parked；compile unused 仍红，**不 push** |
| A | `vscode-WorkTrees/A` | `loop/A` | `bb53e1778aa` | `__pycache__` | 0 | idle |
| B | `vscode-WorkTrees/B` | `loop/B` | `bb53e1778aa` | `__pycache__` | 0 | idle |
| C | `vscode-WorkTrees/C` | `loop/C` | `bb53e1778aa` | 未提交 `dev/loop` + `__pycache__` | 0 | idle；勿 add `dev/loop` |
| D | `vscode-WorkTrees/D` | `loop/D` | `bb53e1778aa` | `__pycache__` | 0 | idle |
| edit | `Projects/Agents/vscode` | `agent-ide` | `a40a95d1e85`+ | `dev/loop` + 过期 progress | 0 | 人类工位；请自行对齐 `bb53e1778aa` |
## Next（Blockers：无）
| 项 | 指针 |
|:---|:-----|
