...
| `AgentService` | `DeleteSnapshot` | `IUniverseAgentConnection.deleteSnapshot?`（unary） | **已进** `UniverseAgentGrpcServices.Agent` + node snake_case `session_id`/`snapshot_id`。响应 `success`/`error_message`。合同可选。空 `sessionId` / 空 `snapshotId` 原样上线。本刀只进 catalog + node 传输；**不**改 SessionBar History / Snapshots overlay / roster |
