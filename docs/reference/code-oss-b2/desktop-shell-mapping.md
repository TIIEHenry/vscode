...
| Navigator body | `SIDEBAR_PART`（Activity 默认标签 **Files**（同一 Explorer `VIEW_CONTAINER`，`isDefault: true`）+ 非 default 容器 Sessions / Projects / Agents / Team） | 保留 Explorer 权威；roster = Files + Sessions + Projects/Agents/Team honest stub；stock Testing（`workbench.view.extension.test`，order 6）**不在** roster，默认窗口 Activity 经 `IsSessionsWindowContext` 隐藏（Agents Window 可保留 Sidebar Testing；Test Results 仍在 Panel） | `toggleRegion('navigatorBody')` ≙ `setSideBarHidden` + 记宽 |
