...
- **Connection Hub 接入分析（2026-09-02，主仓）：** 新建 [connection-hub-client](../plans/connection-hub-client.md) `draft`——对照上游 `connection-hub` 三稿 + ADR-261/318/374/375 + proto、Desktop `apps/desktop/src/main/engine/**`（ADR-025/026/027）与 Singularity `ConnectionResolver` / `HubApiClient`，结论：IDE 扮演 **Client 设备**，接入 = M6-A1 gRPC 宿主上增「Hub 解析 + Device Grant」拨号臂；v1 中继 + DirectAddress、GUA 直连留 v2；refresh 持久化到 `ISecretStorageService`（与 Desktop 不同，因 access 默认 15 min + I4 自动重连）；切片 H0–H6。**规则 16 未跑**，排 W3-h。
...
| W3-h | **规则 16 只读审查**：[connection-hub-client](../plans/connection-hub-client.md)（`draft` @2026-09-02，Hub 接入分析：IDE 为 Client 设备，`platform/universeAgent/node` 增 Hub 解析 + Device Grant 拨号臂，Desktop Main 模块为 donor）；过审后先落 PRD-024 `proposed`（H0），再 H1 crypto / trust / hub http（可与 W1-A 并行、不同文件）；H2–H5 排 W1-A 合入后 | review → 实施 | 稿 `accepted`；KAT-1/2 单测绿 | 独立 reviewer；实施 A 域 |
