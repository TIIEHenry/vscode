...
export interface IUniverseAgentGrpcTransport {
	...
	/** ToolService.ListCommands unary (empty request). Empty name / agent / model / skill_source mapped as-is. */
	...
	/** FileService.ListFiles unary (snake_case `path`/`session_id`/`pattern`/`max_results`). Empty ids sent as-is. */
	...
	/** FileService.GetFileInfo unary (snake_case `path`/`session_id`). Empty ids sent as-is. */
	...
	/** FileService.ForceWriteFile unary (snake_case `path`/`content`/`session_id`). Empty ids sent as-is. */
	...
	/** GitService.ReadGitSummary unary (snake_case `session_id`). Empty ids sent as-is. */
	readGitSummary(request: UniverseAgentReadGitSummaryRequest): Promise<UniverseAgentReadGitSummaryResult>;

	/** GitService.ReadGitChanges unary (snake_case `session_id`). Empty ids sent as-is. */
	readGitChanges(request: UniverseAgentReadGitChangesRequest): Promise<UniverseAgentReadGitChangesResult>;

	/** GitService.ReadGitFileDiff unary (snake_case `session_id`/`path`/`index_state`). Empty ids sent as-is. */
	...
	/** TokenUsageService.GetSessionUsage unary (snake_case `session_id`). Empty ids sent as-is. */
	...
	/** MemoryService.Save unary (snake_case `scope`/`content`/`category`). Empty ids sent as-is. */
	...
	/** ContextVariableService.Read unary (snake_case `session_id`/`name`/`agent_id`). Empty ids sent as-is. */
	...
	/** RemoteAgentService.GetNode unary (snake_case `node_id`). Empty ids sent as-is. */
	getNode(request: UniverseAgentGetNodeRequest): Promise<UniverseAgentRemoteAgentInfo>;

	/** RemoteAgentService.CheckConnection unary (snake_case `node_id`/`session_params`). Empty ids sent as-is. */
	...
	/** RemoteAgentService.DeleteConfig unary (snake_case `node_id`). Empty ids sent as-is. */
	...
	/** RemoteAgentService.SaveConfig unary (snake_case `config`/`skip_connection_test`/`async_test`). Empty ids sent as-is. */
	...
	/** RemoteAgentService.ResumeRemoteSession unary (snake_case `call_id`/`node_id`). Empty ids sent as-is. */
	resumeRemoteSession(request: UniverseAgentResumeRemoteSessionRequest): Promise<UniverseAgentResumeRemoteSessionResult>;
	/** RemoteAgentService.CancelRemoteSession unary (snake_case `call_id`/`reason`). Empty ids sent as-is. */
	...
	/** TriggerService.FireTrigger unary (snake_case `scope`/`scope_id`/`trigger_id`). Empty ids sent as-is. */
	...
	/** ClipboardService.Clear unary (snake_case `session_id`). Empty ids sent as-is. */
	...
	/** ConfigService.Watch server-stream (snake_case `keys`). Empty keys sent as-is. */
...
export const UniverseAgentGrpcServices = {
	...
	RemoteAgent: {
		...
		ResumeRemoteSession: 'ResumeRemoteSession',
