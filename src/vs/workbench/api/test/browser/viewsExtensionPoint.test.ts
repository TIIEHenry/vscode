...
suite('ViewsExtensionPoint - default window Activity', () => {
	...
	test('gates extension views contributed to Activity sidebar containers', () => {
		...
		assert.strictEqual(
			npmWhen!.evaluate({ getValue: <T extends ContextKeyValue = ContextKeyValue>(key: string) => defaultWindow[key as keyof typeof defaultWindow] as T }),
			false,
			'Npm Scripts explorer must not occupy Files in the default Code window'
		);
		assert.strictEqual(
			npmWhen!.evaluate({ getValue: <T extends ContextKeyValue = ContextKeyValue>(key: string) => agentsWindow[key as keyof typeof agentsWindow] as T }),
			true,
