...
phase: M7
updated: 2026-09-14
summary: "2026-09-10 订阅架构审查 1–3 项的实施方案：S1 宿主按 IPC 连接 / 窗口回收 lease；S2 SessionEventStream remote/error 关流后宿主退避重开（登记 G-CORE-2）；S3 gRPC unary deadline + 渲染端 bind 可重试。S1–S3b 代码已落 `b2211b23fa5`；2026-09-14 规则 16 回溯审查已做并改稿。仍 draft：S4a 诊断口未接（V-S1/V-S2 无观察点）、S4b fail-closed 与退避定时器竞态未守、§5 手测未跑（D405）"
---
...
> **不推翻：** [ADR-003](../decisions/003-engine-adapter-boundary.md)（`contrib/**` 禁 import `platform/universeAgent/node/**`）；[session-view-frame-fanout](session-view-frame-fanout.md) F1/F2（per-lease 动态事件、首帧缓冲、`postAndDrain` 唯一入口、`hadSubscriber` 闩）；[conversation-stream-timeline §3](conversation-stream-timeline.md)（lease 是显示写源、末 lease 释放后 linger）；**vendored `node/sessionCore/**` 不手改**（[giant-file-split](giant-file-split.md) GFS-4 PIN）；`IUniverseAgentSessionView` 六方法 + `acknowledge` 合同**不增不减**。  
> **槽位：** 平台合同与宿主，归 **P 槽**；S3 渲染端一刀只改 `conversationEngineFrameSource.ts`。  
> **审查记录：** 见文末（规则 16）。

## 实施现状（2026-09-14 回填）

**S1 / S2 / S3a / S3b 代码已落**，四刀一次性合入 `b2211b23fa5`（2026-09-12，「补会话订阅生命周期」），类型修补 `3c07195f6bf`；HEAD `815b4ad48e3` compile-client 0。§7 知识层回填七件事全部完成。§4 单元 Exit 全绿（新增 `sessionViewHostReopen.test.ts` / `grpcUnaryDeadline.test.ts` / `universeAgentSessionViewChannel.test.ts`）。

**本稿仍为 `draft`**，三件事未闭：

| 未闭 | 性质 |
|------|------|
| **S4a** 生产 `DiagnosticsPort` 是空函数体且 electron-main 未注入 → §5 里所有基于计数的验收在生产里**没有观察点** | 须改代码或改验收口径，否则 D405 无法诚实闭合 |
| **S4b** 「退避定时器已武装 → mailbox 溢出 fail-closed → 定时器触发」竞态会造成**永久假 live** 并静默解除 fail-closed 闩 | 须加守卫 + 回归测；守卫所需信息 core 未暴露，追加进 G-CORE-2 |
| **§5 手测** V-S1–V-S3 一次未跑（[D405](../progress/deferred-gaps.md) 仍 open） | 须隔离 profile + 钉死引擎实跑 |

**§1 行号是基线 `ad9245ca178` 的锚点，HEAD 已漂移 366 提交**，读表时按符号名定位，勿按行号。被 S1–S3 自己改掉的行已就地标注「**已落地**」。
...
| 事实 | 位置 |
|------|------|
| `releaseLease(leaseId)` 是唯一释放入口；`ActiveLease` 无 owner 字段 —— **已落地**：HEAD `ActiveLease.owner?: string`，另有 `releaseLeasesOwnedBy` | `node/sessionViewHost.ts` L88–97 · L253–267（HEAD：`acquireLease` L253 / `releaseLease` L285 / `releaseLeasesOwnedBy` L305） |
...
| 渲染端 `EngineSessionViewLease.ready` 在构造函数一次性计算：`whenEngineSessionReady` reject / 空 id → `false` 永久；`post()` / `requestDetail()` 先 `await this.ready` —— **已落地**：拆成 `acquired`（HEAD L102）+ 可重臂 `bind`（L103） | `conversationEngineFrameSource.ts` L102–139 · L157–167 |
| roster `monitorListedEngineSessionBind` / `monitorPendingEngineSessionBind` 只 await 一次 `whenLeaseBindReady`；bind false 且无其他已绑会话 → `markEngineSessionBindFailed` 占位；恢复只靠下一次 `onUaConnectionChanged` → `doRefreshEngineCatalog` | `conversationEngineRosterService.ts` L1344–1410 · L1440–1480（HEAD L1497 · L1521） |
...
| 取消 | `onEngineConnectionChanged` down 分支清全部；`releaseLease` / `releaseLeasesOwnedBy` 后若 `leaseCount === 0` 清该 session；`dispose` 清全部 |
| 可观察 | `diagnostics.count('stream.reopen_scheduled', { attempt })`、`'stream.reopen_fired'`、`'stream.reopen_skipped', { why }`（`no_lease` / `connection_down` / `attempt_open`）。**注意：生产里这些计数目前全是空转** —— `createSessionViewDiagnosticsPort()`（`sessionViewHostPorts.ts` L69–77）两个方法体皆空，且 `electron-main/universeAgentSessionViewService.ts` L26 构造宿主时未注入替代实现。只有单测注入 `CountingDiagnostics` 才看得见。见 **S4a** |
...
| roster | **源文件不改，但行为会变**：`whenLeaseBindReady`（`conversationEngineFrameSource.ts` L52–55）转调可重臂的 `whenBindReady()`，而 roster 两个监视器每次 `bindSessionView` 都调它（`conversationEngineRosterService.ts` L1497 · L1521），所以一次 bind 失败后**roster 刷新本身**就会发起全新 bind，不再是 await 一个已 settle 的 `ready`。`markEngineSessionBindFailed` 语义不变。「重试」只有在上一次 bind 已 reject、宿主 `ensureEngineSession` 的 `engineBindInflight`（`sessionViewHost.ts` L514–517）在 `finally` 删掉在途条目之后才是真的新尝试 —— 这正是 §4 里 S3b 硬依赖 S3a 的机制 |
...
| **S4b** fail-closed 与退避竞态守卫 | 给 `streamReopenSkipReason` 加第四个 skip 理由（`fail_closed`）。core 未暴露 `subscriptionFailed`（见 §3.2 G-CORE-2 追加诉求），故先用宿主可得信息做**保守近似**：宿主自记「本 session 最近一次 `failClosedOverflow` 的 mailbox 溢出」并在其后停止 reopen，直到下一次 `onAcquireLease` / 连接翻转。若上游先闭合诉求则改为直接查 `subscriptionFailed` | S2 | `sessionViewHostReopen.test.ts` 新增顺序测：remote 关流武装定时器 → 溢出 fail-closed（`currentAttemptId === null`，无 `closeStream` intent）→ 定时器触发时 `reopen_skipped{fail_closed}`，且**不得**有 lease 收到 `sync: live` patch。既有六个 reopen test 保持绿 | `node/sessionViewHost.ts` + 测试 |

S1 / S2 / S3b / S4a / S4b 都改宿主侧，**P 槽单写者串行**：S1 → S2 → S3a → S3b →（S4a ∥ S4b，无文件交集）。`npm run compile`（或 `compile-client`）作每刀门禁。

**实施纪律未按本节执行（记账）：** S1–S3b 四刀一次性合入 `b2211b23fa5`，每刀门禁被跳过，合入后遗留 12 个 compile-client 类型错，由 `3c07195f6bf` 单独修补（只把泛型签名与 `SessionViewHostSetTimeoutFn` / `UnaryDeadlineCallProperties` 显式化，不改语义）。`deferred-gaps.md` D405 行仍写「compile-client 1」，须对齐 HEAD 的 0。
...
| V-S2-2 | 同上但引擎不再回来 | ① 引擎侧观察 `SubscribeSessionEventStream` 到达间隔递增并稳定在 ~30 s（±20% jitter）；② 主进程 `stream.reopen_scheduled` 单调 +1（S4a 前置）；③ 断开 Connection 后 60 s 内不再有重开尝试 |
...
| V-S4b-1 | S4b 落地后：先让流被远端关闭（进入退避窗口），再在窗口内灌满 mailbox 触发 fail-closed | 会话徽标停在 degraded（`mailbox_overflow`），**不得**翻回「已连接」；主进程出现 `stream.reopen_skipped{fail_closed}` |
...
| **（2026-09-14 审查追加）** 退避定时器已武装期间发生 fail-closed → 定时器触发 → `onConnectionUp` 无条件刷 `sync: live` 且清 `subscriptionFailed`，但 `allowOpen === false` 不开流 ⇒ **无订阅的会话被永久画成「已连接」**，且不会再有关流事件来重新调度 | S4b 守卫 + 顺序回归测；G-CORE-2 追加诉求。可达性窄（需在 1–30 s 退避窗口内塞满 256 条 mailbox），但 §3.2 原文的无条件安全断言已撤 |
...
| **（追加）** `openStream` 在 engine id 未绑时只 warn 并 return，**不** post `streamClosed`（`sessionViewHost.ts` L1240–1245），Actor `currentAttemptId` 保持非 null ⇒ `streamReopenSkipReason` 永久返回 `attempt_open`，S2 永久哑掉 | 当前该分支不可达（§2 原判 Minor 9），但**它的不可达性现在是 S2 的承重墙**：谁让它可达就必须同时补 post `streamClosed`。已从「非目标」升为本表 |

## 7. 知识层回填（实施 commit 时）

**状态：** 下列六项已随 `b2211b23fa5` 全部完成（含 `platform/overview.md` 的两件顺手活）。**唯一缺项：本方案文件自己至今未 commit**，导致 HEAD 上已有 5 处指向本稿的悬空链接（`deferred-gaps.md` ×2、`traceability.md` ×1、`engine-protocol-surface.md` ×2，另加工作树未提交的 `dev/plans/INDEX.md`）。补齐时须一并 commit 本稿。
...
**升格条件：** S4a 落地（或 §5 改为纯引擎侧口径）+ S4b 落地 + §5 全部实跑并回填 [D405](../progress/deferred-gaps.md) 后，方可从 `draft` 升 `accepted` / `implemented`。
