...
created: 2026-08-30
updated: 2026-09-07
summary: "延期缺口 SSOT；D16 Lens 断言债；D22 F3；D15 欠 W1；D23 heartbeat；D24 其余 JSON RPC；D25 ghost UI；D26 引擎建壳回 6；D27 Lens 编辑态整树重建；D28 Connection 旁路 UI；D29 inbox closed；D30 Inbox FAILED Retry closed；D31 Sources git 次级面；D32 retry lease；D33 pairing chip；D37 引擎 roster 队列 Retry unary"
---
...
| D33 | P2 | **配对中 chip 可能误开 Engine 页**：`conversationSessionStatus.ts` `getEngineStatusCommandId` 只看 `phase.connected`，忽略 `pairingPending` | 决策核实时发现；号原误写成 D24（该号已是其余 JSON RPC） | chip 在 pairingPending 时指向 Connection/SAS，不打开 Engine | M7 conversation | open |
