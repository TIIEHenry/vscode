...
| [D405](deferred-gaps.md)–[D472](deferred-gaps.md) | A/D 本波 | D405 手测仍开；D472 已闭；**D24/D471 仍开**（五 unary 已 bytes、Sessions stub；剩 listTools JSON / split/fence）；catalog 死 `onOpenConnection` 不叠 CTA |
## 工位表（P0 盘点 · 2026-09-16 · 与 `git worktree list` 对照）
| 槽 | 路径 | 分支 | tip | 脏 | stash | 关仓状态 |
|----|------|------|-----|:--|:------|:---------|
| merge | `vscode-WorkTrees/merge` | `loop/merge` | MERGE_SHA | 干净 | 0 | A D24 + D D471 已合；compile-client 0；87 passing；两号仍开 |
...
| D | `vscode-WorkTrees/D` | `loop/D` | MERGE_SHA | 干净 | 0 | `idle`；ff-only 或 `-B` 跟 MERGE_SHA；D471 仍开 |
