...
created: 2026-08-30
updated: 2026-09-04
summary: "延期缺口 SSOT；D16 进口界扫 src + deleteTurn 已隔离；D19/D21 已闭；D22 F3；D15 欠 W1"
---
...
| D16 | P2 | **conversation 单测基线红**：Lens DOM/codicon 等仍红；identity 1 条。stub `deleteTurn` 已改走 `createSession()`（不再把 7 条 fixture 的 `untitled` 当空）。**进口界空扫假绿已收**（四份测改扫 `src/` + `>=80`） | 启动级夹具已修；本行剩 Lens / identity 断言债 | Lens / identity 逐条分类；S2「Lens 全绿」以本行闭合为前提，或改为「不新增失败」 | M6 / conversation | open |
