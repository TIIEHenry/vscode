...
connect(profileId)
 1. resolver：
    hubDevice → GET /devices（live）
        ...
        401/403 → `withHubAccessRetry` 强制 `refreshIfNeeded({ force: true })` 后重试一次（@ `23fd9d70`）；refresh 仍 401/403 或无 token → `hub_auth_expired`（v1 无直连 ⇒ 直接 failed；v2 可快照直连、不签 ticket）
...
| refresh + csrf | **加密持久化**（main 侧等价 secret 存储，见 §3.6 落点行；加密不可用则内存、并诚实提示「重启需重登」）；`POST /auth/refresh` 单飞；**启动时** access 过期则 `restorePersistedHubSessionIfNeeded` → `refreshIfNeeded`（@ `dba63c70`）；**运行中** access 过期或控制面 401/403 则 `withHubAccessRetry` → `refreshIfNeeded({ force: true })` 单次重试（@ `23fd9d70`） | **与 Desktop 不同、与 Singularity 相同**：Hub access 默认 **15 min**，IDE 进程长命且 I4 自动重连每次要新 ticket；不持久化 refresh 会让每次网络抖动都变成「请重新登录」 |
...
| **H4a** **已落** @ `127c0586` | pane 四区 + SAS 对话框；ProxyChannel 增 hub 面；**不改** StatusBar。**残留已落：** SAS 确认/取消接到 `confirmPairing`/`cancelPairing`（GC-1b）；设备 Rename/Revoke/设备码（GC-2）；Test Connection → `probeConnectionProfile`（GC-3）；**recoverTrust**（S4）pane 独立 identity+fingerprint 对话框（Desktop ADR-031；0× SAS；确认仍 `confirmPairing`） | `connectionPreferencesPane.test.ts` 扩：矩阵六行文案、SAS 无第三按钮、Hub 登录态不改 `isEngineConnected`；recoverTrust 确认走指纹对话框；`conversationSessionStatusBar` 既有测加一条负向断言「Hub signedIn + `connected` 前，entry 文案仍 `Engine not connected`」；隔离 profile 冒烟对**真 Hub**（`connection-hub` 本地 `go run` + Engine `--hub --enroll`）：登录 → 列表 → SAS → 连接 → roster 出现 UA 会话（需 M6-A2 已合入；否则只验到 `connected`） | `contrib/conversation/browser/connectionPreferencesPane*.ts`、`common/uaPreferencesPanes.ts`、`electron-browser` 代理 |
