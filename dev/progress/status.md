...
- **槽 B / `loop/B`：** 把 `AgentService.Compact` 写入 gRPC catalog（`universeagent.agent.v1.AgentService`），node unary `compact`（snake_case `session_id`/`agent_id`；响应 `success`/`message`/`tokens_before`/`tokens_after`/`outcome`/`reject_reason`）。合同可选。空 `sessionId` / `agentId` 原样上线。**不改** Conversation roster / SessionBar / Snapshots overlay。≠ `Todo` / `Status` / `Tree`。测：catalog + 转发 / 空 id。
...
| B | `vscode-WorkTrees/B` | `loop/B` | AgentService.Compact catalog + node unary compact；空 sessionId / agentId 原样上线；无 Conversation UI |
