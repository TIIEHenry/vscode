...
| D42 | P3 | **Maximize 把整个 `.conversation-timeline` 槽 `display:none`**（`conversationLens.css`），轨迹页与对话树同槽，最大化后切 Trajectory 虚拟列表不铺行 | B 槽已改 CSS：只藏 `.conversation-lens-timeline`；轨迹可见时不整槽 `display:none`。T5a 去掉 `display:block` + `revealRecord` 夹具并补铺行测 | 最大化后切 Trajectory 仍能见行；或 CSS 只藏 `.conversation-lens-timeline` 不藏轨迹 | conversation | closed |
