...
phase: M7
updated: 2026-09-16
summary: "C 已写 GetConfig wire（endpoint/auth/delegate/health unread）未接线。七条 loop/tool 已 bytes；其余 RemoteAgent wire 未接线。D24 仍开。compile-client 未跑。"
---
...
| 槽 | 状态 |
|:---|:-----|
| **C** | C 已写 GetConfig wire（endpoint/auth/delegate/health unread）未接线。仍勿关本行。脏 `dev/loop` 勿 add |
...
| ID | 问题 |
|:---|:-----|
| [D24](deferred-gaps.md) | **仍开**：七条 loop/tool **已** bytes；GetConfig/ListConfigs/SendShell/ListNodes/GetNode/Maintenance/ResetError wire 未接线；ResolveTurn/Connect/SaveSkillContent 仍 JSON |
...
| 项 | 指针 |
|:---|:-----|
| **loop** | D24 仍开。C 已写 GetConfig wire（endpoint/auth/delegate/health unread）未接线。仍勿关本行。A 下一刀接 SendShell 与 RemoteAgent 七条 wire。SaveSkillContent 无 RPC。Connect/Watch/ResolveAnchor 跳过。不得宣称 leftover/pills 完成 |
