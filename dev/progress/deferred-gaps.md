...
created: 2026-08-30
updated: 2026-09-07
summary: "延期缺口 SSOT；D16 Lens 断言债；D22 F3；D15 欠 W1；D23/D32 已由 A 槽 host-write-retry 收口；D24 其余 JSON RPC；D25 ghost UI；D26 引擎建壳回 6；D27 编辑态 1px 垫高 closed；D28 Connection 旁路 UI；D29 inbox closed；D30 Inbox FAILED Retry closed；D31 Sources git 次级面；D33 pairing chip closed；D37 引擎 roster 队列 Retry unary；D38 fillHistory 无 catch"
---
...
| D38 | P3 | **`void fillHistory` / `ensureEngineSession` 无 catch**：`sessionViewHost.ts` ~917 / ~1232 与已修 D23 同类，bind 失败可成未处理 rejection | A 本刀只收 heartbeat；不扩 HistoryFill | `fillHistory` 路径 bind/write 与 `writeChat` 同级 catch；补测 | M7 universeAgent | open |
...
| D37 | P3 | **引擎 roster 未 override `retryMessageQueueItem`** 到 `AgentService.RetryQueueItem` / `RetryQueueItemUpload`。接通后无 GetQueue，失败行本就不可见 | overlay 面 API 已落 stub；A 已释放 roster 文件 | `ConversationEngineRosterService` 接通后按 `upload` 转发对应 unary；空 item / 断连缓存 / 无 hook 不发；补 roster 测 | conversation | open |
