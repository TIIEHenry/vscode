...
| `AgentService` | `Chat` | 发送 + 流内 question / clientTool 应答（Chat 双向流） | 权限座接通后改走 `PermissionService.Respond` unary（不双写 Chat 臂）；未接通仍 `permissionRespond`。question 传输 `respondQuestion?` **已进** `AgentService.RespondQuestion` catalog，时间线 / roster **未**转发，仍 Chat 臂 `questionRespond`。clientTool 接通后改走 `AgentService.SendClientToolResponse` unary（不双写 Chat 臂）；未接通仍 `clientToolRespond` |
...
| `AgentService` | `RespondQuestion` | `IUniverseAgentConnection.respondQuestion?`（unary） | **已进** `UniverseAgentGrpcServices.Agent` + node snake_case `session_id` + nested `response.question_id`/`answers`（`selected_labels`）/`custom_text`。响应 `success`/`error`。合同可选。空 `questionId` 原样上线。**未**转发 roster / 时间线问题座（仍 Chat 臂 `questionRespond`；≠ Permission.Respond / clientTool） |
