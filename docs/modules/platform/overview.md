...
phase: N/A
updated: 2026-09-15
summary: "platform 层角色、可导入范围、DI 约定；universeAgent 会话帧走 onDynamicDidApplyFrame(leaseId) 宿主缓冲；lease 按 owner 回收；unary 默认 deadline；生产 DiagnosticsPort 落主进程日志；fail-closed 后停止流级 reopen；bind 等 whenEngineSessionReady（Create/Resume），不把 acquireLease / web-empty 当成功"
---
...
- `IUniverseAgentConnection`（`common`）：Connect 生命周期、`isEngineConnected`（`session_token` + 活 channel）、`getConnectionPhase()`、`getCapabilitySnapshot()`（`grpcCapabilityProbe`：`skills` / `agentProfiles` / `mcp` / `mcpRuntime` / `tools` / `plugins` / `models` / `agentTree` / `team`）、session / chat / team / **catalog list** 面、**`confirmPairing` / `cancelPairing` / `probeConnectionProfile`**（GC-1b / GC-3）；可选 `openChatStream` 常驻 Chat bidi；unary 默认 30s deadline（流/bidi 不加）；`DEADLINE_EXCEEDED` 后成功 unary 且仍 connected 则相位回到 `connected`。renderer 经 electron-main ProxyChannel 代理
- `IUniverseAgentSessionView`（`common`）：`acquireLease` / `whenEngineSessionReady`（Create/Resume）/ `releaseLease` / `post` / `requestResync` / `acknowledge`（→ Actor `frameAck`）/ `requestDetail`；帧事件为 **`onDynamicDidApplyFrame(leaseId)`**（F1）：宿主按 lease 过滤并在首个订阅者前缓冲。已删除全窗 `onDidApplyFrame`。宿主按 IPC owner 回收 lease（连接移除 / 窗口销毁）；electron-main 注入打点 `DiagnosticsPort`（`count`/`warn` 落主进程 `ILogService`）。SessionEventStream remote/error 关流后宿主退避重开；mailbox 溢出 fail-closed 后停止 reopen 直至新 lease / 连接翻转。Web `acquireLease` 仍可发 `web-empty:` id，但 `whenEngineSessionReady` 拒绝，不得把 `web-empty:` 当 bind 成功。宿主 `node/sessionViewHost.ts`：`drainIntents` 按 attempt/timer/chat id 归属会话；无 snapshot 时 `overlayDeltaJoin` 累积 L3 delta
