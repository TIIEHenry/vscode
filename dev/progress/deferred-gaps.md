...
| D24 | P2 | **其余 unary/stream 仍 `makeUnaryClient` + `JSON.stringify`**（Info/ChatSync/clipboard/mcp/memory/queue/`listTools` 等）。接通后 catalog 已 bytes：`Session.List` / `ListModels` / `ListAgents` / `ListAgentProfiles`。`SessionEventStream` 握手未写 `client_id`、无 60s heartbeat uplink。`CreateSessionRequest` proto 无 `title`（本仓仍只传 `model`，未写 `client_session_id`）。`Agent.Chat` 响应只解 `session_id`/`agent_id`，未 demux ChatResponse oneof | 本轮只修 Create `ALREADY_EXISTS` 恢复 + composer catalog 那几条；ChatSync 是 Gateway 专用 | 按实际打到引擎的 method 再改 bytes；`listTools` 仍 JSON | platform / universeAgent | open |
