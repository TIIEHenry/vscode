...
created: 2026-08-30
updated: 2026-09-04
summary: "延期缺口 SSOT；D15 前置 P0+E2-1 已落只欠 W1 冒烟；D17–D21 / D8 / D9 / D12 / D16 不冻结 UI 主线"
---
...
| D21 | P2 | **host 暴露 Agent 树首拉失败态**（GC-5b；对齐 [navigator-engine-segments §3](../plans/navigator-engine-segments.md)「首拉失败后停在正在读取」禁止句）：renderer 拿不到 `AgentService.Tree` 首拉失败事实；host `scheduleAgentTreeRefresh` 失败只在 capability 上区分 UNIMPLEMENTED。连接 ok、首拉失败、树一直 `undefined` 时 Hierarchy / Team 会停在「正在读取」。共用空态函数已预留 `treeFetchFailed?: boolean`，**信号 HEAD 缺失** | 归订阅流 Actor 回路；不把 gRPC `transport === 'failed'` 错轴复制到 Team | host 经 lease 快照或 connection 事件提供 `treeFetchFailed`；两叶共用函数接该输入后失败 note 优先于「正在读取」 | M7 navigator | open |
