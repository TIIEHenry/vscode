...
- **槽 C / `loop/C`：** 把 `AgentService.CreateSnapshot` 写入 gRPC catalog（`universeagent.agent.v1.AgentService`），node unary `createSnapshot`（snake_case `session_id`/`title`/`description`；响应 `success`/`snapshot`/`error_message`）。合同可选。空 `sessionId` / 空 title 原样上线。**不改** SessionBar History / roster（History 仍回合索引；Delete snapshot 未进）。测：catalog + 转发 / 空 id / 失败映射。此前 turnEdit 转发 EditMessage。
- **槽 D / `loop/D`：** 把 `AgentService.CreateSnapshot` 写入 gRPC catalog（`universeagent.agent.v1.AgentService`），node unary `createSnapshot`（snake_case `session_id`/`title`/`description`；响应 `success`/`snapshot`/`error_message`）。合同可选。空 `sessionId` / 空 `title` 原样上线。**不改** SessionBar History / roster（History 仍回合索引）。Delete snapshot 未进。测：catalog + 转发 / 空 id / 失败映射。此前 RespondQuestion catalog（问题座转发已由 B 合入）。

## 槽位（与 `git worktree list` 对照）

| 槽 | 路径 | 分支 | 状态 |
|----|------|------|------|
| merge | `vscode-WorkTrees/merge` | `loop/merge` | SessionBar Snapshots 只读 listSnapshots + RestoreSnapshot catalog + CreateSnapshot catalog + respondClientTool→SendClientToolResponse + 问题座→RespondQuestion + AutoDrive 诚实空 + deleteTurn DeleteMessage + turnEdit EditMessage + ToolInfo + Respond + Enqueue + Kill / MessageQueue |
...
| D | `vscode-WorkTrees/D` | `loop/D` | AgentService.CreateSnapshot catalog + node unary；Delete snapshot 未进 |
