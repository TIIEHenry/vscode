...
phase: M7
updated: 2026-09-16
summary: "合入 A/F/G D24 wires + C–J D482–D486。D24 仍开（Memory/File grpcClient JSON）；D482–D486 closed；D487 open。不是 leftover/pills 完成。R9/D405 仍开"
---
...
## Current Session
### 已合入（`MERGE_SHA` 本关仓提交；`npm run compile-client` 0；聚焦测 92 passing）
| 切片 | 提交 / 落点 |
|:-----|:------------|
...
| **agent-ide.yml** | `cc6bfd25` — [test-baseline-ci](../plans/test-baseline-ci.md) 切片 4：compile / eslint / docs-health / unit-custom 四 job；**2026-09-10 永久关闭**（[DISABLED](../../.github/workflows/DISABLED.md)） |
...
| **A D24 + D D481** | `b7df6c535e7`/`25898088f8b` — respondQuestion/sendClientToolResponse bytes；Reveal CI void catch。[D481](deferred-gaps.md) **closed**。[D24](deferred-gaps.md) **仍开**。工位 D [D482](deferred-gaps.md) WSL void catch（未合）。不是 leftover/pills 完成 |
...
### 进行中（2026-09-16 · A–J 合入中；D24 仍开；不是 leftover/pills 完成）
| 槽 | 切片 | 状态 |
|:---|:-----|:---------|
| **A** | D24 Tool leftover unaries bytes | SkillInfo/SetSkillEnabled/ToolInfo/ListCommands/GetCommandDef（+DeleteAgentProfile）转 bytes。[D24](deferred-gaps.md) **仍开**。不是 leftover/pills 完成 |
| **F** | D24 Memory unary wire | 未接线 grpcClient；[D487](deferred-gaps.md) **open**。不是 leftover/pills 完成 |
| **G** | D24 File unary wire | 未接 grpcClient；勿 add `out`。不是 leftover/pills 完成 |
| **C** | D483 createNewSession void catch | [D483](deferred-gaps.md) **closed**。脏 `dev/loop` gitlink 勿 add |
| **D** | D482 WSL void catch | [D482](deferred-gaps.md) **closed**（四处 `.catch`）。不是 leftover/pills 完成。不得把 F 写成 D482 |
| **B** | — | 脏 `worktree-pool.md` 跳过 `-B` |
| **I** | D485 history `void refresh` catch | [D485](deferred-gaps.md) **closed**。[D24](deferred-gaps.md) **仍开**（Memory / ChatSync JSON）。不是 leftover/pills 完成 |
| **J** | D486 snapshots void catch | 调用点 `.catch(onUnexpectedError)`；mocha restore reject + paint 再抛 0 unhandled。[D486](deferred-gaps.md) **closed**。[D24](deferred-gaps.md) **仍开**（Memory / ChatSync JSON）。不是 leftover/pills 完成 |
| **B/C** | — | B 脏 `worktree-pool.md` 跳过 `-B`；C 脏 `dev/loop` gitlink 勿 add、试 ff-only |
| **E** | leftover `fix/ci-gate-reds` | `blocked`；勿 `checkout -B` |
| **H** | D484 sessionArtifacts `_actions` void catch | **closed**；四处双链 `.catch(onUnexpectedError)`（无内层 catch，对齐 D480）；copy reject + handler 再抛 → 0 unhandled。未 compile-client。[D24](deferred-gaps.md) **仍开**。不是 leftover/pills 完成 |
| **A/D** | D24 bytes + D481 | 已合；D24 仍开（Memory / ChatSync JSON）；D481 closed |
| **B/C/E** | 脏树 / leftover | B 脏 `worktree-pool.md` 勿 `-B`；C 脏 `dev/loop` gitlink 勿 add；E leftover `blocked` |
<details>
...
| [D24](deferred-gaps.md) | A bytes；F Memory wire；G File wire | **仍开**：Tool leftover 已 bytes；Memory/File unary wire 未接线 grpcClient；ChatSync 仍 JSON |
...
| [D405](deferred-gaps.md)–[D486](deferred-gaps.md) | A/F/G/C/D/H/I/J 本波 | D405 手测仍开；D471–D486 已闭；**D24 仍开**（Memory/File grpcClient 仍 JSON）；[D487](deferred-gaps.md) **open**；不得宣称 leftover/pills 完成 |
...
| A | `vscode-WorkTrees/A` | `loop/A` | `6af71063c05` | 干净 | 0 | D24 Tool leftover 已合；D24 仍开 |
| B | `vscode-WorkTrees/B` | `loop/B` | `28ffd1ae9f2` | 脏 `worktree-pool.md` | 0 | 跳过；勿 `-B` |
| C | `vscode-WorkTrees/C` | `loop/C` | `5202d99f3b4` | 脏 `dev/loop` gitlink | 0 | D483 已合；ff-only 保持 gitlink 脏；勿 add |
| D | `vscode-WorkTrees/D` | `loop/D` | `e8813c10003` | 干净 | 0 | D482 WSL catch 已合；D24 仍开 |
| E | `vscode-WorkTrees/E` | `fix/ci-gate-reds` | `41f0d8c912f` | 干净 | 0 | `blocked` leftover；跳过 |
| F | `vscode-WorkTrees/F` | `loop/F` | `891c2c22a29` | 干净 | 0 | Memory unary wire 未接线 grpcClient；D24 仍开；D487 open |
| G | `vscode-WorkTrees/G` | `loop/G` | `2c57e7be17f` | File wire | 0 | File unary wire+测；未接 grpcClient；D24 仍开；勿 add `out` |
| H | `vscode-WorkTrees/H` | `loop/H` | `f988e140ea8` | 干净 | 0 | D484 artifacts 已合；D24 仍开 |
| I | `vscode-WorkTrees/I` | `loop/I` | `cb1a6f4abce` | 干净 | 0 | D485 history 已合；D24 仍开 |
| J | `vscode-WorkTrees/J` | `loop/J` | `27743a2648f` | 干净 | 0 | D486 snapshots 已合；D24 仍开 |
...
| **loop 切片** | A/F/G D24 wires 未关 D24（Memory/File grpcClient 仍 JSON）。C [D483](deferred-gaps.md) **closed**。D [D482](deferred-gaps.md) **closed**。H [D484](deferred-gaps.md) **closed**。I [D485](deferred-gaps.md) **closed**。J [D486](deferred-gaps.md) **closed**（snapshots void refresh/restore/delete catch）。[D487](deferred-gaps.md) **open**。R9 / **D405** 仍开。不关 D8/D16/D147。未升 PRD-008。不得宣称 leftover / pills 完成。 |
