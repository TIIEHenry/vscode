...
47. **B 槽 `sessions-roster-engine-connect`（未关 D16）**：Sessions view 补 `onDidChangeEngineConnection` → `refreshList()`。接通后 `getSessions()` 空则列表立即空，不再留 stub 行。[D78](deferred-gaps.md) **已闭**。
