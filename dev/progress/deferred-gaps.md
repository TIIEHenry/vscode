...
created: 2026-08-30
updated: 2026-09-07
summary: "延期缺口 SSOT；D16 Lens 断言债；D22 F3；D15 欠 W1；D23/D32/D38/D39/D40 已由 A 槽收口；D24 其余 JSON RPC；D25 ghost UI；D26 host 已收、引擎建壳回 6 仍开；D27 编辑态 1px 垫高 closed；D28 Connection 旁路 UI；D29/D30 inbox closed；D31 Sources git 次级面（F4 仍开）；D33 pairing chip closed；D34–D36 timeline hygiene closed；D37 引擎 roster 队列 Retry unary closed；D41 Inbox FAILED/UPLOAD_FAILED 行 class 分流 closed"
---
...
| D16 | P2 | **conversation 单测基线红**：Lens 夹具已补（sessionBar 实测宽 + `lens.layout` 叶宽 + ResizeObserver）。过时断言已改：删非末会话计数 −1；300px 验 `is-narrow` 不是像素；permission seat aria 走 `Permission` 合同。**identity 已绿**（10/10）并改走共享 `conversationLensLayoutHarness`。**全部 Lens 夹具套已隔离**（`conversationLensLayoutHarness`：ResizeObserver loop 拦截 + layout flush，接到 conversationLens / identity / reveal / trajectory / trajectoryUi；after-each 同式 flush）。stub `deleteTurn` 已改走 `createSession()`。**进口界空扫假绿已收**（四份测改扫 `src/` + `>=80`）。`conversationLens.test.ts` 仍可能有 DOM/codicon 断言债 | 启动级 / identity / stub / 进口界 / Lens 叶宽夹具 + 全套共享 harness + 上述过时断言已修；本行剩 Lens 断言债 | `conversationLens.test.ts` 逐条分类；S2「Lens 全绿」以本行闭合为前提，或改为「不新增失败」 | M6 / conversation | open |
...
| D31 | P3 | **Sources git 次级面**：Review 列表读失败 / Panel 写门控已在前刀。本刀（`sources-leftover-honesty`）收 Changes 已暂存 git 行 Unstage 不可用诚实态（`resolveSourcesChangesRowAction` + `sourcesGitUnstageUnavailableMessage`）与 Review 打开 diff 空 catch（`sourcesGitDiffOpenFailureMessage` 走 status，不标已审）。未发明 `WriteGitUnstage` / 未改空 patches。**F4 隔离 profile 冒烟未跑**；`sourcesReviewCommands.contribution.ts` Open Selected 仍空 catch。不升 PRD-009/023 | 禁 F4 / 禁升 PRD；R8 空 patches 不本刀发明 | F4 V-F1–V-F7 证据目录；R8 定稿后再决定 Accept 是否传 path | sources-git | open |
