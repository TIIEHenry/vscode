...
- **槽 merge：** 合入 A 的接通后 `respondClientTool` 转发 `AgentService.SendClientToolResponse`（空 callId 不发；未接通仍 Chat 臂）、B 的接通后问题座转发 `AgentService.RespondQuestion`（空 questionId 不发；未接通仍 Chat 臂）、C / D 同片的 `AgentService.CreateSnapshot` catalog（node unary；空 sessionId / title 原样上线；SessionBar History 仍回合索引；Restore / Delete 未进）。ListSnapshots catalog、C 此前时间线 `updateUserTurnText` / `turnEdit` 转发 `AgentService.EditMessage`、D 此前 `RespondQuestion` catalog 已在 tip。Inbox AutoDrive 诚实空 / DeleteMessage / ToolInfo / Respond / Enqueue / Kill / MessageQueue 已在 tip。
- **槽 A / `loop/A`：** 接通后时间线 / roster `respondClientTool` 转发已进 catalog 的 `AgentService.SendClientToolResponse`（空 `callId` / 未知 session / 断连缓存 / 无 hook 不发；不双写 Chat 臂）。未接通 / stub 仍写本地 `clientToolRespond`。测：catalog 转发 / 空 id / 断连。此前 SendClientToolResponse catalog + Inbox AutoDrive 诚实空。
...
- **槽 C / `loop/C`：** 把 `AgentService.DeleteSnapshot` 写入 gRPC catalog（`universeagent.agent.v1.AgentService`），node unary `deleteSnapshot`（snake_case `session_id`/`snapshot_id`；响应 `success`/`error_message`）。合同可选。空 `sessionId` / 空 `snapshotId` 原样上线。**不改** SessionBar History / roster（History 仍回合索引；Restore snapshot 未进）。测：catalog + 转发 / 空 id / 失败映射。此前 CreateSnapshot catalog。
...
| 槽 | 路径 | 分支 | 状态 |
|----|------|------|------|
| merge | `vscode-WorkTrees/merge` | `loop/merge` | respondClientTool→SendClientToolResponse + 问题座→RespondQuestion + CreateSnapshot catalog + ListSnapshots catalog + AutoDrive 诚实空 + deleteTurn DeleteMessage + turnEdit EditMessage + ToolInfo + Respond + Enqueue + Kill / MessageQueue |
| A | `vscode-WorkTrees/A` | `loop/A` | 接通后 respondClientTool 转发 SendClientToolResponse；空 callId 不发；未接通仍 Chat 臂 |
