...
suite('grpc RemoteAgentService DestroyRemoteSession protobuf wire', () => {
	...
	test('destroyRemoteSession uses bytes then existing map; skip Connect/SaveSkillContent/Watch/ResolveTurn', () => {
		...
		assert.ok(destroy.includes('makeUnaryBytesClient'), 'destroyRemoteSession must use makeUnaryBytesClient');
		assert.ok(destroy.includes('encodeDestroyRemoteSessionRequest'), 'destroyRemoteSession must call encodeDestroyRemoteSessionRequest');
		assert.ok(destroy.includes('decodeDestroyRemoteSessionResponse'), 'destroyRemoteSession must call decodeDestroyRemoteSessionResponse');
		...
		assert.ok(!destroy.includes('makeUnaryClient<'), 'destroyRemoteSession must not use JSON makeUnaryClient');
		assert.ok(!destroy.includes('JSON.stringify'), 'destroyRemoteSession must not JSON.stringify');
