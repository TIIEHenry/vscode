...
phase: M7
updated: 2026-09-04
summary: "Conversation SessionBar Snapshots 只读列表消费 listSnapshots（断连 / 无 hook / 空 sessionId 不发；History 仍回合索引）；接通后时间线 / roster respondClientTool 转发 SendClientToolResponse；问题座转发 RespondQuestion；Inbox AutoDrive 诚实空；deleteTurn / turnEdit 转发 DeleteMessage / EditMessage；Engine Tools 选中行拉 ToolInfo；权限座转发 Respond；CancelToolCall / Kill / MessageQueue / Enqueue 已转发；Composer 发送仍走 submitInput"
---
...
- **槽 merge：** 合入 A 的接通后 `respondClientTool` 转发 `AgentService.SendClientToolResponse`、B 的接通后问题座转发 `AgentService.RespondQuestion`。ListSnapshots catalog、C 的 `turnEdit`→`EditMessage`、D 的 `RespondQuestion` catalog 已在 tip。
- **槽 A / `loop/A`：** 接通后时间线 / roster `respondClientTool` 转发 `AgentService.SendClientToolResponse`（空 `callId` 不发；未接通仍 Chat 臂）。
- **槽 B / `loop/B`：** Conversation SessionBar extra control 只读引擎 snapshot 列表（`listSnapshots?`；id / title / created_at / turn_count）。断连 / 无 hook / 空 sessionId 不发，诚实空 / unavailable，不用 fixture 冒充引擎数据。**不**改 SessionBar History（仍 `conversationTrajectoryList.ts` 回合索引）。无 Create / Restore / Delete。测：conversation browser。
