...
phase: N/A
updated: 2026-09-05
summary: "八份治理方案签收 accepted；HEAD @ 40d61319677 实况：Wave 1 多项已落（GC-1b、GFS-1–4、packaging P0、D1、U0、U1、docs-burden S1–S5、test-baseline 切片 4 workflow）；P1 gulp 与切片 0/1/3 本工位未跑"
---
...
**跨方案实施顺序（签收裁定；**`HEAD` @ `40d61319677`** 实况见下节）：**

1. **Wave 1（可并行）**：GC-1b 配对回路 ∥ PRD-008 E0 ∥ test-baseline 切片 0–4 ∥ packaging P0 ∥ GFS-1a/1b + GFS-2 + GFS-3 + GFS-4 sync ∥ cross-repo D1（docs-only）∥ ADR-007 U0（docs-only）+ U1 只读准备。
2. **Wave 2**：PRD-008 E1–E5（等 GC-1b）；packaging P1–P4；prd-020 B0/B1（等 test-baseline 切片 1）。
3. **Wave 3**：ADR-007 U2 第一次专项合入（等 U0 `comm` 为空 + U1 结论落盘 + test-baseline 切片 4 CI 绿；独占 merge 槽、冻结 A 表文件）；PRD-008 E6 / prd-020 B3（知识层升格）。
...
`HEAD` = `40d61319677`（`loop/merge`）。下列为 present-tense 实况，SHA 经 `git merge-base --is-ancestor` 核对在 `HEAD` 上。
...
| **GFS-4** | `a4ab1a3e754` | vendored session-core sync from Desktop `0dd3146cd`（PIN `0dd3146cd05efdcce118d0c3c7e19eb28b615f5c`）；G6 @ `48cd90952` |
