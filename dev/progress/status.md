...
phase: M7
updated: 2026-09-16
summary: "merge compile-client 0；聚焦测 320 passing。D479 closed；D24 仍开（MCP/plugins JSON）。不是 leftover/pills 完成。R9/D405 仍开"
---
...
| 槽 | 切片 | 状态 |
|:---|:-----|:---------|
| **A/D** | listTools/listSkills + D479 | A：`listTools`/`listSkills` unary bytes、[D24](deferred-gaps.md) **仍开**（MCP/plugins/clipboard JSON）；D：turn-changes 四处 void catch、[D479](deferred-gaps.md) **closed**。compile-client **0**；聚焦 **320 passing / 0 fail / 11 pending**。不是 leftover/pills 完成 |
...
| [D24](deferred-gaps.md) | A 槽 `d24-listtools-listskills-bytes` | **仍开**：`listTools`/`listSkills` 已 bytes；MCP / plugins / clipboard / respondQuestion 仍 JSON |
...
| [D405](deferred-gaps.md)–[D479](deferred-gaps.md) | A/D 本波 | D405 手测仍开；D471–D479 已闭；**D24 仍开**（listTools/listSkills 已 bytes；MCP/plugins/clipboard 仍 JSON）；不得宣称 leftover/pills 完成 |
...
| D | `vscode-WorkTrees/D` | `loop/D` | MERGE_SHA | 脏 D479 | 0 | [D479](deferred-gaps.md) **closed**（四处 catch）；D24 仍开；勿宣称 leftover/pills 完成 |
...
| **loop 切片** | compile-client **0**；聚焦 **320 passing / 0 fail**。A listTools/listSkills bytes 已合。[D24](deferred-gaps.md) **仍开**（MCP/plugins/clipboard JSON）。[D479](deferred-gaps.md) **closed**（工位 D，四处 void catch）。R9 仍开。**D405** 手测仍开。不关 D8/D16/D147。未升 PRD-008。不得宣称 leftover / pills 完成。 |
