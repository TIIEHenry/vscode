...
created: 2026-08-30
updated: 2026-09-08
summary: "延期缺口 SSOT；D16 仍开；D45–D90 已闭；D22 F3；D24 其余 JSON RPC；D25 引擎 List 真空；D26 引擎建壳回 6；D31 F4 / A2 blocked；Fork openForkTab / Connection CRUD / ensurePrimaryWindow throw 已挂"
---
...
| D90 | P3 | **`ensurePrimaryWindow` 在 `ensureLeaf` 成功前写入 `primarySessionKey`**：`createConversationEditorPart` / `whenReady` throw 后半应用 primary（key 已设、leaf 可能在 map、DOM 已挂）且 `void` 调用成未处理 rejection | B 槽 `session-window-primary-bootstrap-catch` 已收：先 `ensureLeaf` 再提交 key；try/catch 回滚半应用 leaf；void 调用点未改。测锁 throw 无 unhandled rejection、key 仍空、随后成功 bootstrap。未关 D16；未改 fork / breadcrumb / Connection pane | catch 后 `getPrimarySessionKey()` 为空；半应用 leaf 回滚；后续 `ensurePrimaryWindow` 可成功；`conversationSessionWindowSideBySide.test.ts` throw 测绿 | conversation | closed |
