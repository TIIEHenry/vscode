...
phase: M7
updated: 2026-09-16
summary: "工位 A Device PairApprove/PairReject/ListPending/RotateToken 已 bytes；ListDevices 不重做。D24 仍开。不是 leftover/pills 完成。"
---
...
### 进行中（2026-09-16 · 工位 A Device pair bytes；D24 仍开）
| 槽 | 切片 | 状态 |
|:---|:-----|:---------|
| **A** | Device pair bytes | PairApprove / PairReject / ListPending / RotateToken **已** `makeUnaryBytesClient`。ListPending 空 proto 非 JSON `{}`。RotateToken reserved 2 不编不解。ListDevices 不重做。[D24](deferred-gaps.md) **仍开** |
...
| ID | 来源 | 问题 |
|:---|:-----|:-----|
| [D24](deferred-gaps.md) | A | **仍开**：SetPermissionPolicy + Doctor + Device PairApprove/PairReject/ListPending/RotateToken **已** bytes；SaveSkillContent/Rebuild/Watch/model prefs 仍 JSON |
...
| 项 | 指针 |
|:---|:-----|
| **loop 切片** | [D24](deferred-gaps.md) **仍开**（SaveSkillContent/Rebuild/Watch/GetModelPreferences/SetModelPreferences/ResolveModel 仍 JSON；Device pair 四条本刀已 bytes）。[D508](deferred-gaps.md)–[D514](deferred-gaps.md) **closed**。[D487](deferred-gaps.md) **open**。不得宣称 leftover / pills 完成 |
