 ...
 * `openDownloadAttachmentStream`, `openRemoteChatStream`, `openPtyStream`,
...
/**
 * RemoteAgentService.ListNodes — proto `ListNodesRequest` /
 * `ListNodesResponse` + `RemoteAgentInfo` / `Capabilities` /
 * `ModelInfo` / `LoadMetrics` only. Empty `filter_status` /
 * `filter_tags` (incl. empty strings) pass through as-is. Empty
 * `id` / `name` / `description` / `status` / `endpoint` / `tags` /
 * `server_version` / `protocol_version` / model `id`/`name`/`provider`
 * mapped as-is. `enabled` false mapped as-is. `last_heartbeat_at` /
...
export interface UniverseAgentListNodesRequest {
	readonly filterStatus: readonly string[];
	readonly filterTags: readonly string[];
}

export interface UniverseAgentListNodesResult {
...
/**
 * RemoteAgentService.SetMaintenance — proto `SetMaintenanceRequest` /
 * `SetMaintenanceResponse` only. Empty `node_id` (incl. empty string)
 ...
 */
export interface UniverseAgentSetMaintenanceRequest {
	readonly nodeId: string;
}

export interface UniverseAgentSetMaintenanceResult {
	readonly success: boolean;
}
 ...
 */
export interface UniverseAgentResetErrorRequest {
	readonly nodeId: string;
}

export interface UniverseAgentResetErrorResult {
	readonly success: boolean;
}
...
/**
 * RemoteAgentService.CreateRemoteSession — proto `CreateRemoteSessionRequest` /
 * `CreateRemoteSessionResponse` + `SessionParams` only. Empty `node_id` /
 * `mode` (incl. empty string) pass through as-is. Empty `preferred_model` /
 * `required_tools` (incl. empty strings) / session `mode` /
 * `system_prompt_suffix` pass through as-is. `max_tokens` / `max_turns` /
 * `max_execution_time_ms` 0 pass through as-is. Empty `call_id` / `status`
 * mapped as-is. `created_at` / `expires_at` 0 mapped as-is.
 ...
 */
export interface UniverseAgentCreateRemoteSessionRequest {
	readonly nodeId: string;
	readonly mode: string;
	readonly sessionParams: UniverseAgentRemoteSessionParams;
}

export interface UniverseAgentCreateRemoteSessionResult {
	readonly callId: string;
	readonly status: string;
	readonly createdAt: number;
	readonly expiresAt: number;
...
/**
 * RemoteAgentService.CancelRemoteSession — proto
 * `CancelRemoteSessionRequest` / `CancelRemoteSessionResponse` only.
 * Empty `call_id` / `reason` (incl. empty string) pass through as-is.
 * `success` false mapped as-is. Empty `call_id` / `status` / `message`
 * mapped as-is.
 ...
 */
export interface UniverseAgentCancelRemoteSessionRequest {
	readonly callId: string;
	readonly reason: string;
}

export interface UniverseAgentCancelRemoteSessionResult {
	readonly success: boolean;
	readonly callId: string;
	readonly status: string;
	readonly message: string;
...
/**
 * TriggerService.SetTriggerEnabled — proto `SetTriggerEnabledRequest` /
 * `SetTriggerEnabledResponse` + `TriggerDto` / `DeliveryTargetDto` only.
 * Empty `scope` / `scope_id` / `trigger_id` pass through as-is.
 * `enabled` false sent as-is. Empty `trigger_id` / `name` / `type` /
 * `prompt_template` / `pause_reason` / `cron_expression` mapped as-is.
 * `enabled` false mapped as-is. `interval_ms` / `run_at_epoch_ms` 0
 * mapped as-is. Delivery oneof `self` / `bound_session` / `new_session`;
 * empty `session_id` / `engine_profile_id` mapped as-is.
 * ≠ ListTriggers / UpsertTrigger / DeleteTrigger / FireTrigger /
 * AgentService.FireTriggerWebhook / DeviceService.
 */
export interface UniverseAgentSetTriggerEnabledRequest {
	readonly scope: string;
	readonly scopeId: string;
	readonly triggerId: string;
	readonly enabled: boolean;
...
export interface UniverseAgentSetTriggerEnabledResult {
	readonly trigger: UniverseAgentTrigger;
}
