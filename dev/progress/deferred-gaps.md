...
created: 2026-08-30
updated: 2026-09-07
summary: "延期缺口 SSOT；新增 D23 heartbeat_ack fire-and-forget；D16 Lens 断言债仍开；D22 F3；D15 欠 W1"
---
...
| D23 | P2 | **`sendHeartbeatAck` fire-and-forget 无总 catch**：`sessionViewHost.ts` `void this.sendHeartbeatAck(sessionId)`；resident `write({heartbeat_ack:{}})` 无 try/catch（`connection.chat` 一路有）。bind/write 失败会成未处理 rejection | A 槽核实时发现；本波未扩 F3、不并进已齐的全局帧删除刀 | `sendHeartbeatAck` 全程与 `writeChat` 同级 catch；`void` 调用点不再漏 rejection；补测 | M7 universeAgent | open |
| D24 | P2 | **配对中 chip 可能误开 Engine 页**：`conversationSessionStatus.ts` `getEngineStatusCommandId` 只看 `phase.connected`，忽略 `pairingPending` | 决策核实时发现；本波 C 在做 grant finalize，不并进 | chip 在 pairingPending 时指向 Connection/SAS，不打开 Engine | M7 conversation | open |
| D29 | P2 | **`enqueueMessageQueueItem` 无 Inbox/Composer 入队 UI**；断连缓存拒收。断连 Send 已接到该 API | Inbox 队列头 Enqueue 已落地 `b384a1dcf9e` | Inbox/Composer 能看见入队项 | UI / conversation | closed |
...
| D31 | P3 | Sources 次级面：Review 读失败静默回 SCM；Panel Diff 无 Stage/Accept/Revert；无 WriteGitUnstage；FileDiff 改 URI 后 Revert 找不到 SCM | A 槽本刀只收主路径假绿；禁 F4 | Review 失败可见；Panel 与对话窗同门控；Unstage 有 hook 或明示不可用 | sources-git | open |
| D32 | P3 | error retry 不经 `lease.post` / Actor `continueGeneration`：UI 走 roster→`openContinuationStream`，宿主 `continuationStreams` 不知这条流 | D 槽禁改 `sessionViewHost.ts` | WriteMessage 增加 `continueGeneration` 且 host 映射到 localFact；UI 改走 `lease.post` | M7 conversation | open |
