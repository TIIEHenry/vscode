...
phase: M7
updated: 2026-09-04
summary: "接通后 killSubAgent 转发 AgentService.Kill（空 agent 不默认 root）；Fork 已转发；ToolInfo / CancelToolCall / Kill 已进 catalog；时间线尚未转发 CancelToolCall；队列 UI 仍 fixture；Create / Rename / Cancel / SetSessionGoal 已转发"
---
...
- **槽 A / `loop/A`：** 接通后 roster `killSubAgent` 转发已进 catalog 的 `AgentService.Kill`（未知 session / 断连缓存 / 无 hook 不发；空 `agentId` 原样上线、**不**默认 `root`；≠ Cancel）。不改 Navigator（仍只读无 Kill 钮）。此前接通后 Fork 转发 / Inbox Goal。
...
| A | `vscode-WorkTrees/A` | `loop/A` | 接通后 killSubAgent 转发 AgentService.Kill；空 agent 不默认 root |
