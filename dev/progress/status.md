...
phase: M7
updated: 2026-09-08
summary: "D16 / D31 F4 仍开。官方三域 glob 已绿。D44–D90 / D92–D129 已闭。接通 forkSubAgent false 已 notice+handled；RotateToken / Revoke success false 已 banner。A2 仍 blocked。"
---
...
62. **A 槽 `connection-revoke-success-false-notice`（未关 D16 / 未关 D31 / 未重做 D61–D65 / D74 / D89 / D128 rotate）**：`handleRevokeSelectedDevice` 引擎 hook `!success` 走 `writeStatus` error + 显示 banner（空 message 用 `connectionDeviceRevokeFailureMessage`）；不 refresh。`success` 仍 refresh + 原 banner。throw 未改。Hub fallback `revokeDevice` 未改。[D129](deferred-gaps.md) **已闭**。未 compile。
子 agent 发现的既有代码问题：
| ID | 来源 | 问题 |
|:---|:-----|:-----|
...
| [D92](deferred-gaps.md)–[D129](deferred-gaps.md) | B roster+inbox+sessions / D nav / A beside+mru+sessionbar / A·B composer / A notifications / B sessionChat / D bind / A navigator lease / B reveal / A attribution / A rotate / A revoke | **closed** throw 回滚/notice；peek；closeEditors；opener 回滚；stale；save/delete/cancel/Stop/Goal/Enqueue false 先 notice；reveal acquire；open-beside / primary bootstrap catch error notice；Goal / Enqueue 断连+history 可点；permission/question roster false；SessionBar rename/delete 与 Sessions 侧栏 delete false notice；notifications / live-tree / bind / navigator lease / Review attribution acquire throw → notice；Create Snapshot false notice；timeline reveal / accessible content lens throw → notice；接通 `forkSubAgent` false notice+handled；RotateToken / Revoke success false banner；未关 D16 |

## 工位表（P0 盘点 · 2026-09-08 · 与 `git worktree list` 对照）
| 槽 | 路径 | 分支 | tip | 脏 | stash | 关仓状态 |
|----|------|------|-----|:--|:------|:---------|
| merge | `vscode-WorkTrees/merge` | `loop/merge` | `e2c9ac540f0` | `__pycache__` | 0 | parked；compile unused 仍红，**不 push** |
| A | `vscode-WorkTrees/A` | `loop/A` | `e2c9ac540f0` | `__pycache__` | 0 | idle |
| B | `vscode-WorkTrees/B` | `loop/B` | `e2c9ac540f0` | `__pycache__` | 0 | idle |
| C | `vscode-WorkTrees/C` | `loop/C` | `e2c9ac540f0` | 未提交 `dev/loop` + `__pycache__` | 0 | idle；勿 add `dev/loop` |
| D | `vscode-WorkTrees/D` | `loop/D` | `e2c9ac540f0` | `__pycache__` | 0 | idle |
| edit | `Projects/Agents/vscode` | `agent-ide` | `a40a95d1e85`+ | `dev/loop` + 过期 progress | 0 | 人类工位；请自行对齐 `e2c9ac540f0` |
## Next（Blockers：无）
| 项 | 指针 |
|:---|:-----|
