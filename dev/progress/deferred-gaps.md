...
| D42 | P3 | **Maximize 后切 Trajectory 虚列表铺行**。CSS 合同已收（只藏 `.conversation-lens-timeline`，轨迹页加 `conversation-lens-showing-trajectory` 不整槽 `display:none`）。铺行路径本刀已改：ResizeObserver / `layoutReadingSurfaces` 忽略 0 尺寸，避免 maximize 收槽后 `layout(0)` 卸行；`show` 后按上次正尺寸再 `list.layout`；T5a 铺行测在产品断言后 `revealRecord('untitled-u1')`（无 `display:block`）。**未跑 compile** | 待 merge compile-client 后跑 T5a 铺行测 | 最大化后切 Trajectory 仍能见 `untitled-u1` 行（merge compile 复证） | conversation | open |
