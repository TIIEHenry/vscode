...
| D31 | P3 | **Sources git 次级面**：Review 列表读失败 / Panel 写门控已在前刀。`sources-leftover-honesty` 收 Changes 已暂存 git 行 Unstage 不可用诚实态与 Review 列表打开 diff 失败。本刀（`review-open-selected`）收 `sourcesReviewCommands.contribution.ts` Open Selected 空 catch：失败走 `sourcesGitDiffOpenFailureMessage` + `ISourcesReviewListHost.setStatusMessage`，不标已审。未发明 `WriteGitUnstage` / 未改空 patches。**F4 隔离 profile 冒烟未跑**。不升 PRD-009/023。[R8](research-queue.md) **已闭**。Accept **产品选项 A 已裁定、未实施**：宿主拒空送（空 session 或空 patches 不调 RPC）；真 path/hunks 须与 session 同刀。见 [sources-accept-empty-success](../plans/sources-accept-empty-success.md)（`draft`）与 [ADR-008](../decisions/008-write-git-apply-hunks-empty.md) Consequences 补记 | 禁 F4 / 禁升 PRD；方案刀不实施 Accept | F4 V-F1–V-F7 证据目录；Accept A1（宿主拒空）实施后单测锁门；A2 真载荷另刀 | sources-git | open |
