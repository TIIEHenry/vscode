...
phase: M7
updated: 2026-09-04
summary: "Engine Tools 选中行拉 ToolService.ToolInfo 只读详情（无 schema 编辑器）；接通后时间线权限座转发 PermissionService.Respond（不双写 Chat 臂，未接通仍走 Chat 臂）；CancelToolCall 空 agent 用末条 streaming 否则 root；Kill roster 已转发；MessageQueue 五操作 + Edit + Enqueue 接通后 roster 转发（无 GetQueue 显示诚实空；空正文 / 未知 session / 断连缓存不发 Enqueue）；Composer 发送仍走 submitInput；Fork / Goal / Create / Rename / Cancel 已转发"
---
...
- **槽 merge：** 合入 A 的 Engine Tools ToolInfo 只读详情与 Kill roster、B 的权限座 `PermissionService.Respond` 转发（未接通仍 Chat 臂）、C 的 `enqueueMessageQueueItem` 转发 `AgentService.EnqueueQueueItem`（空正文不发；Composer 发送仍走 `submitInput`）。Respond catalog、MessageQueue 五操作 + Edit 已在 tip。
...
- **槽 D / `loop/D`：** 接通后 roster 转发已进 catalog 的 MessageQueue 五操作 + `EditQueueItem`（未知 session / 空 item / 空正文 / 断连缓存不发）。`getMessageQueueState` 接通后诚实空（无 GetQueue，不把 fixture 冒充引擎队列）。不改 CancelToolCall 时间线。此前 Kill catalog。

## 槽位（与 `git worktree list` 对照）

| 槽 | 路径 | 分支 | 状态 |
|----|------|------|------|
| merge | `vscode-WorkTrees/merge` | `loop/merge` | ToolInfo 只读详情 + Respond roster 转发 + Enqueue + Kill / MessageQueue roster |
...
| D | `vscode-WorkTrees/D` | `loop/D` | 接通后 MessageQueue 五操作 + Edit 转发；无 GetQueue 显示空 |
