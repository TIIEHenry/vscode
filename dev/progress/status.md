...
phase: M5
updated: 2026-09-02
summary: "loop/merge 集成：D11 closed；D8 open（TS lib）；D9 partial；R6 closed @ ADR-005；R5 closed @ m6 + ADR-003 accepted；M6-A ready"
---
...
- **PRD-008：** `blocked`（活数据）；方案 [m6-engine-wave](../plans/m6-engine-wave.md) + [ADR-003](../decisions/003-engine-adapter-boundary.md) **`accepted`** → **M6-A ready**（见 Next）
...
| 1 | ~~**R5 签收**~~ [m6-engine-wave](../plans/m6-engine-wave.md) + [ADR-003](../decisions/003-engine-adapter-boundary.md) — **closed** @ 2026-09-02 | review | ✅ 两文 `accepted`；[stream-timeline §9](../plans/conversation-stream-timeline.md) 已并入 | B |
| 1a | **M6-A adapter**（[m6 §8](../plans/m6-engine-wave.md#m6-a--adapter本波新建) + [stream-timeline S4–S5](../plans/conversation-stream-timeline.md)） | 实施 | **ready**：platform/universeAgent + 同 token 替换 stub；Connect / List / history fold / fail-closed 发送链 | A |
...
| 4 | **M6 引擎波实施**（R5 closed）：~~M6-A~~ → page-access 切片 5 → customizations E1 → trajectory T4 → Engine pane 能力探测 | 实施 | M6-A **ready**（工位 A）；各方案 blocked 行转 `implemented`；D4 式隔离 profile 验收 | A–D 并行 |
