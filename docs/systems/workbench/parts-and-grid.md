...
| `SOURCES_PART` | `workbench.parts.sources` | **End 列下格**：`contrib/sources` **Files \| Changes \| Review** tab strip（Files = 只读列表投影；Changes = SCM 变更资源列表 + **stage/unstage/commit** 列表操作 → Preview；Review = SCM 变更资源只读列表 → Preview（`SourcesReviewList`）；树权威仍在 Sidebar Explorer） |
