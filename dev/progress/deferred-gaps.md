...
created: 2026-08-30
updated: 2026-09-07
summary: "延期缺口 SSOT；D16 Lens 断言债仍开（visualize 三测 + remainder 7 已收，全量 97/97，本行不闭）；D22 F3；D15 欠 W1；D23/D32/D38/D39/D40 已由 A 槽收口；D24 其余 JSON RPC（probeRpc 已 empty proto；listTools 仍 JSON）；D25 host leftover 已收、引擎 List 真空仍开；D26 host 已收、引擎建壳回 6 仍开；D27 编辑态 1px 垫高 closed；D28 Connection 旁路 UI closed（身份条布局预留 + 几何测；(2)(3) 仍闭）；D29/D30 inbox closed；D31 Sources git 次级面（Open Selected 空 catch 已收，F4 仍开）；D33 pairing chip closed；D34–D36 timeline hygiene closed；D37 引擎 roster 队列 Retry unary closed；D41 Inbox FAILED/UPLOAD_FAILED 行 class 分流 closed"
---
...
| D37 | P3 | **引擎 roster 未 override `retryMessageQueueItem`** 到 `AgentService.RetryQueueItem` / `RetryQueueItemUpload`。接通后无 GetQueue，失败行本就不可见 | C 槽 `roster-queue-retry` 已 override 并补 roster 测；无 GetQueue 仍使接通后失败行不可见 | `ConversationEngineRosterService` 接通后按 `upload` 转发对应 unary；空 item / 断连缓存 / 无 hook 不发；补 roster 测 | conversation | closed |
...
| D31 | P3 | **Sources git 次级面**：Review 列表读失败 / Panel 写门控已在前刀。`sources-leftover-honesty` 收 Changes 已暂存 git 行 Unstage 不可用诚实态与 Review 列表打开 diff 失败。本刀（`review-open-selected`）收 `sourcesReviewCommands.contribution.ts` Open Selected 空 catch：失败走 `sourcesGitDiffOpenFailureMessage` + `ISourcesReviewListHost.setStatusMessage`，不标已审。未发明 `WriteGitUnstage` / 未改空 patches。**F4 隔离 profile 冒烟未跑**。不升 PRD-009/023 | 禁 F4 / 禁升 PRD；R8 空 patches 不本刀发明 | F4 V-F1–V-F7 证据目录；R8 定稿后再决定 Accept 是否传 path | sources-git | open |
