...
phase: M7
updated: 2026-09-08
summary: "FileMutationJoin / createScoped / Review 委托已修。D16 / D31 F4 仍开。官方三域 glob 已绿。D44–D90 / D92–D99 / D102 已闭。sessions MRU opener reject 回滚已挂。A2 仍 blocked。"
---
...
61. **B D92–D99 / A D102**（未关 D16）：roster 回滚；split notice；beside restore/闸门；conversation goBack/goForward 回栈 + notice；sessions `_navigateTo` opener reject 回滚 `_currentKey` + warn。
子 agent 发现的既有代码问题：
| ID | 来源 | 问题 |
|:---|:-----|:-----|
...
| [D92](deferred-gaps.md)–[D99](deferred-gaps.md)/[D102](deferred-gaps.md) | B roster / D split+nav / A beside+mru | **closed** remote fail 回滚；split notice；beside restore/闸门；conversation 回栈 + notice；sessions opener reject 回滚 cursor；未关 D16 |
