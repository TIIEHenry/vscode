...
created: 2026-08-30
updated: 2026-09-02
summary: "P2/P3 延期缺口 SSOT；D1–D7、D11、D10、D13、D14 已闭；D8 valid-layers 环境红降 P3 并豁免为门禁 / D9 terminal 行非阻塞 / D12 产品身份（引擎波后）/ D15 PRD-019 Web 冒烟 / D16 conversation 单测基线红 / D17 I3b 三平台打包未验"
---
...
| D17 | P2 | **I3b 三平台安装包未验**：`gulpfile.vscode.linux.ts` / rpm spec 已把 I3a `resources/linux/icons/hicolor/{16,24,32,48,64,128,256,512}x{size}/apps/universe-agent-studio.png` 列入 deb/rpm；`electron.ts` 已改 `companyName`/`copyright`/`darwinHelpBook*`；未跑 `vscode-linux-*-prepare-deb` / `prepare-rpm` / snapcraft / Inno / darwin 打包，故任务栏小尺寸、Windows 向导位图、macOS icns/HelpBook 均无产物证据 | I3b 委派单明确先不复杂测试、不跑完整 gulp 三平台打包；本机也缺 fakeroot/rpmbuild/Inno/macOS | V 槽（或有工具工位）跑 Linux `prepare-deb`/`prepare-rpm` 确认 `.build/linux/**/usr/share/icons/hicolor/*/apps/universe-agent-studio.png` 八档都在；Windows/macOS 至少检查产物内 `code.ico`/`inno-*.bmp`/`code.icns` 与 exe/plist 元数据为本产品 | product / packaging | open |
