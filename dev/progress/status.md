...
50. **A 槽 `lens-retry-error-postbound-catch`（未关 D16 / 未碰 composer switchModel / sessions view / channel client / navigator / proto / `dev/loop`）**：`retryError` 的 `postBound` reject 走 `.catch` → `showPostFailure('failed')`；第五 reason + localize，不映射四因。[D80](deferred-gaps.md) **已闭**。未 compile。
51. **B 槽 `channel-client-hydrate-catch`（未关 D16 / 未发明 proto·GetQueue·heartbeat client_id）**：`UniverseAgentConnectionChannelClient` `hydrate` / `refreshPhaseAndNotify` 包 try/catch；IPC reject 保留已写缓存、不 fire 半应用 phase。测锁无未处理 rejection。[D81](deferred-gaps.md) **已闭**。未 compile。
