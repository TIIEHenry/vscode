...
created: 2026-08-30
updated: 2026-09-07
summary: "延期缺口 SSOT；D16 Lens 断言债；D22 F3；D15 欠 W1；D23 heartbeat closed；D24 其余 JSON RPC；D25 ghost UI；D26 引擎建壳回 6；D27 编辑态 1px 垫高（树重建已落）；D28 Connection 旁路 UI；D29 inbox closed；D30 Inbox Retry closed；D31 Sources git 次级面；D32 retry lease closed；D33 pairing chip；D34–D36 timeline 卫生；D37 引擎 roster 队列 Retry unary；D38 fillHistory 无 catch"
---
...
| D23 | P2 | **`sendHeartbeatAck` fire-and-forget 无总 catch**（A 槽已 catch bind + resident write + chat，待 commit） | A 槽核实时发现 | `sendHeartbeatAck` 全程与 `writeChat` 同级 catch；`void` 调用点不再漏 rejection；补测 | M7 universeAgent | closed |
| D38 | P3 | **`void fillHistory` / `ensureEngineSession` 无 catch**（`ba622ce1173` 已补 catch + 测，未进 merge） | — | bind/write 与 writeChat 同级 catch；补测 | M7 universeAgent | closed |
| D39 | P3 | **`requestDetail` await `ensureEngineSession` 无 catch**（`ba622ce1173` 已补，未进 merge） | — | bind 失败回 `{ ok:false }`；补测 | M7 universeAgent | closed |
| D40 | P3 | **`requestDetail` bind 成功后 `fetchToolDetail` 仍无 catch**（`687e98ee172` 已补 fetch catch + 测） | A 槽 `request-detail-fetch-catch` | throw → `{ok:false,reason:'failed'}`；host `{ok:false}` 原样返回 | M7 universeAgent | closed |
| D41 | P3 | Inbox `FAILED` 与 `UPLOAD_FAILED` 行共用 `upload-failed`（`5fa7c8a8d9f` 已拆 `queue-failed` / `upload-failed`） | C 槽 `inbox-fail-class` | 两状态行 class 互斥；FAILED 不再标 `upload-failed` | conversation | closed |
...
| D26 | P2 | **引擎建壳回 6**：host Tree 非 UNIMPLEMENTED 可观察、Create recover 单层 **已收**（`ba622ce1173`，未进 merge）。引擎 Create 建目录后按目录存在回 6 且不写 `session_meta` 仍开 | 禁止改引擎仓 | 引擎 Create 先写 meta 再回成功 | engine | open |
| D27 | P3 | **Lens 编辑态 1px 垫高**：整树 `setChildren` 已收口。`provideTurnEditComposer` 在 `setEditingTurnId` 前把 composer 交给 renderer；已删 `minHeight = '1px'`（`826346878a3`） | 工位 B `lens-edit-host` 已进 merge | 渲染时 composer 已在 host 内，再删 1px 下限后 `conversationLens.test.ts` 仍无 ListView 0px console | M6 / conversation | closed |
| D34 | P3 | **`lastVisibleElement` 无已布局行时 throw**（`687e98ee172` 已用 `renderHeight`/`flatItems` 守卫） | 工位 B `timeline-hygiene` | 空树与 `layout(0)` 不抛 | M6 / conversation | closed |
| D35 | P3 | **死的第二次 `querySelector`**（`687e98ee172` 已删） | 工位 B `timeline-hygiene` | 只留一条 `[data-turn-id], [data-fold-id]` | M6 / conversation | closed |
| D36 | P3 | **独立 thinking/tool 行仍是 stub**（`687e98ee172` 已改为诚实摘要） | 工位 B `timeline-hygiene` | 无假 process-fold chrome | M6 / conversation | closed |
...
| D30 | P3 | **Inbox FAILED 行无重试钮**（时间线 retryable 行已接 ContinueGeneration @ `583b004a8b0`） | C 槽 overlay Retry → `retryMessageQueueItem` 已落（待字母槽 commit）。引擎 unary 见 D37 | Inbox FAILED / UPLOAD_FAILED 行有 Retry，点击转发 roster API，失败不退化成死徽章 | conversation | closed |
| D37 | P3 | **引擎 roster 未 override `retryMessageQueueItem`**（`7b8d9c39c7e` 已 override 并补测，未进 merge）。接通后无 GetQueue，失败行在活引擎上仍不可见 | 列表 RPC 不本刀发明 | roster 按 `upload` 转发；空 item / 断连 / 无 hook 不发；补 roster 测 | conversation | closed |
| D31 | P3 | **Sources git 次级面**：Changes Unstage 不可用、Review 列表/命令开 diff 失败上浮 **已进 merge**（`5fa7c8a8d9f`）。**F4 隔离 profile 冒烟未跑**；不升 PRD-009/023 | 禁 F4 / 禁升 PRD；R8 空 patches 不本刀发明 | F4 V-F1–V-F7 证据目录；R8 定稿后再决定 Accept 是否传 path | sources-git | open |
| D32 | P3 | **error retry 不经 lease.post / Actor `continueGeneration`**（A 槽已加 Write 臂 + host 映射 + Lens `postBound`；roster 不再直开流，待 commit） | 测待 merge compile | `ConversationWriteMessage` 增加 `continueGeneration` 且 host 映射到 localFact；UI 改走 `lease.post`；补 host 流句柄与 UI 打开不打架的测 | M7 conversation | closed |
| D33 | P2 | **配对中 chip 可能误开 Engine 页**（B 槽已改 `getEngineStatusCommandId(phase, pairingPending)`，待字母槽 commit） | 决策核实时发现；号原误写成 D24 | chip 在 pairingPending 时指向 Connection/SAS，不打开 Engine | M7 conversation | closed |
