...
`ConversationPart` 实现：`src/vs/workbench/browser/parts/conversation/conversationPart.ts`。它 **不是** `EditorInput` / Custom Editor / Sidebar ViewPane。Part 只暴露 `sessionBar` + `sessionWindowGrid` 槽；`contrib/conversation` 在叶内工厂 Conversation `IEditorPart`（`EditorParts.createConversationEditorPart`）并在 `ConversationEditorPane` 填 timeline/dock 页 chrome，`browser/parts` 不 import contrib。
...
**推论（INV-TOPO）：** Layout 中心叶 **必须**是 `CONVERSATION_PART`，禁止把中心改回 `Parts.EDITOR_PART` 或用 `ChatEditor` / Custom Editor 当产品对话。默认 `openEditor` / `ACTIVE_GROUP` / `SIDE_GROUP` 下，文件永远进 End 列 Preview — **唯一登记例外**：经显式动作带入的只读 Diff 审阅 input（`ConversationDiffReviewInput`）可进 Conversation 组延伸 tab（[ADR-005](../../../dev/decisions/005-changes-diff-owner.md)；[sources-changes-diff](../../../dev/plans/sources-changes-diff.md) F1–F3 已落）。ADR-002 **第四类 editor 容器**已落：`ConversationEditorPartImpl` 挂在 ConversationPart 的 session 叶内（`createConversationEditorPart` 注册进 `IEditorGroupsService.parts`、共用主窗 `windowId`），只接受 conversation 类 input，且 **`excludeFromGlobalEditorAggregation`** 退出全局枚举、MRU `activePart`、`applyState` 工作集恢复与 editor 历史（见 [editor-part-tabs](editor-part-tabs.md) §4）。S0 拒绝的是 **ChatEditor 占中心**，不是「Conversation 内部不能有 EditorGroup」。
