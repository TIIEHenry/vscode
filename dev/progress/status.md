...
58. **D 槽 `fork-open-tab-throw-notice`（未关 D16）**：`_tryForkAsChat` / `_openForkedSession` 的 `openForkTab` try/catch → notice；accessor 在首个 await 前 hoist。[D88](deferred-gaps.md) **已闭**。
子 agent 发现的既有代码问题：
| ID | 来源 | 问题 |
|:---|:-----|:-----|
